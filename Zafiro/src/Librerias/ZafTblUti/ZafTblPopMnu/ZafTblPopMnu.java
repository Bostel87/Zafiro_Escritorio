 ...
 * @author  Eddye Lino
 */
public class ZafTblPopMnu extends javax.swing.JPopupMenu
{
    //Constantes:
    ...
    private static final int INT_MNU_COP_AVA=3;                 /**Menú en el que se dió click: Indica "Copia avanzada".*/
    ...
    private static final int INT_MNU_MAR_CHK_CEL_SEL=7;         /**Menú en el que se dió click: Indica "Marcar casillas de verificación".*/
    private static final int INT_MNU_DES_CHK_CEL_SEL=8;         /**Menú en el que se dió click: Indica "Desmarcar casillas de verificación".*/
    private static final int INT_MNU_INS_FIL=9;                 /**Menú en el que se dió click: Indica "Insertar fila".*/
    private static final int INT_MNU_INS_VAR_FIL=10;            /**Menú en el que se dió click: Indica "Insertar filas".*/
    private static final int INT_MNU_ELI_FIL=11;                /**Menú en el que se dió click: Indica "Eliminar fila(s)".*/
    private static final int INT_MNU_BOR_CON=12;                /**Menú en el que se dió click: Indica "Borrar contenido".*/
    private static final int INT_MNU_OCU_COL=13;                /**Menú en el que se dió click: Indica "Ocultar columna".*/
    private static final int INT_MNU_MCO_TOD_COL=14;            /**Menú en el que se dió click: Indica "Mostrar columna: Todas las columnas".*/
    private static final int INT_MNU_AJU_ANC_COL=15;            /**Menú en el que se dió click: Indica "Ajustar ancho de columnas automáticamente".*/
    private static final int INT_MNU_TSE_CEL=16;                /**Menú en el que se dió click: Indica "Tipo de selección: Selección por celda".*/
    private static final int INT_MNU_TSE_FIL=17;                /**Menú en el que se dió click: Indica "Tipo de selección: Selección por fila".*/
    private static final int INT_MNU_TSE_COL=18;                /**Menú en el que se dió click: Indica "Tipo de selección: Selección por columna".*/
    private ZafCopAva objCopAva;
    ...
    private javax.swing.JMenuItem mnuCor, mnuCop, mnuPeg, mnuCopAva;
    private javax.swing.JMenuItem mnuSelFil, mnuSelCol, mnuSelTod;
    private javax.swing.JMenuItem mnuMarChkCelSel, mnuDesChkCelSel;
    ...
    private int intColTblCliButDer;                             //Columna en la tabla donde se dió click con el botón derecho del mouse.
    private int intNumTotFilIns;                                //Número total de filas insertadas.
    private int intFilEli[];                                    //Filas eliminadas.
    
    /**
     * Crea una nueva instancia de la clase ZafTblPopMnu.
     * @param tabla El objeto JTable sobre el cual trabajará el ZafTblPopMnu.
     */
    public ZafTblPopMnu(javax.swing.JTable tabla)
    {
        this.tblDat=tabla;
        intNumDec=6;
        blnCanCli=false;
        objTblMod=(ZafTblMod)tblDat.getModel();
        objCopAva=new ZafCopAva(null, true, tblDat);
        //Crear los menúes.
        ...
        mnuCopAva=new javax.swing.JMenuItem("Copia avanzada...");
        ...
        mnuMarChkCelSel=new javax.swing.JMenuItem("Marcar casillas de verificación...");
        mnuMarChkCelSel.setEnabled(false);
        mnuDesChkCelSel=new javax.swing.JMenuItem("Desmarcar casillas de verificación...");
        mnuDesChkCelSel.setEnabled(false);
        ...
        this.add(mnuSelTod);
        this.addSeparator();
        this.add(mnuMarChkCelSel);
        this.add(mnuDesChkCelSel);
        this.addSeparator();
        this.add(mnuInsFil);
...
//            }
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                intColTblCliButDer=tblDat.columnAtPoint(evt.getPoint());
                tblDatMouseReleased(evt);
            }
        });
        ...
        mnuCopAva.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                mnuCopAvaActionPerformed(evt);
            }
        });
        ...
        mnuMarChkCelSel.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                mnuMarChkCelSelActionPerformed(evt);
            }
        });

        mnuDesChkCelSel.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                mnuDesChkCelSelActionPerformed(evt);
            }
        });
        ...
        //Asignar el PopupMenu a la tabla.
        tblDat.setComponentPopupMenu(this);
    }
    ...
    {
        //Así funcionaba hasta Java 6.
//        if (evt.isPopupTrigger())
//            this.show(evt.getComponent(),evt.getX(),evt.getY());
        //Así funciona en Java 8.
//        if(evt.getButton()>=2)
//            this.show(evt.getComponent(),evt.getX(),evt.getY());
    }
    ...
    private void mnuCorActionPerformed(java.awt.event.ActionEvent evt)
    {
        ...
        try
        {
            ...
            //Permitir/Cancelar la edición de acuerdo a "cancelarClick".
            if (!blnCanCli)
            {
    ...
    private void mnuPegActionPerformed(java.awt.event.ActionEvent evt)
    {
        ...
        int intFilSel[], intColSel[], i, j, m, n, intFil, intCol;
        int intNumFilCop, intNumColCop, intNumFilSel, intNumColSel;
        ...
        try
        {
            ...
            if (!blnCanCli)
            {
                ...
                if (objTra!=null && objTra.isDataFlavorSupported(java.awt.datatransfer.DataFlavor.stringFlavor))
                {
                    ...
                    intNumFilCop=stkDat.countTokens();
                    intNumColCop=stkDat.nextToken().split("\t").length;
                    intNumFilSel=intFilSel.length;
                    intNumColSel=intColSel.length;
                    stkDat=null;
                    if (intNumFilSel==1 && intNumColSel==1)
                    {
                        //Permitir pegar sólo si está seleccionada una celda.
                        stkDat=new java.util.StringTokenizer(strDat,"\n",false);
                        for (i=0; stkDat.hasMoreTokens(); i++)
                        {
                            ...
                            {
                                intFil=i + intFilSel[0];
                                intCol=j + intColSel[0];
                                if ( objTblMod.isCellEditable(intFil, intCol) && isValPegVal(strFil[j], intFil, intCol) )
                                {
                                    objTblMod.setValueAt(strFil[j], intFil, intCol);
                                }
                            }
                        }
                    }
                    else if ( intNumFilSel==1 && (intNumColSel%intNumColCop)==0 )
                    {
                        //Permitir pegar sólo si están seleccionadas varias columnas (con la misma forma del área copiada) y una sola fila.
                        for (n=0; n<(intNumColSel/intNumColCop); n++)
                        {
                            stkDat=new java.util.StringTokenizer(strDat,"\n",false);
                            for (i=0; stkDat.hasMoreTokens(); i++)
                            {
                                ...
                                {
                                    intFil=i + intFilSel[0];
                                    intCol=j + intColSel[0] + n*intNumColCop;
                                    if ( objTblMod.isCellEditable(intFil, intCol) && isValPegVal(strFil[j], intFil, intCol) )
                                    {
                                        objTblMod.setValueAt(strFil[j], intFil, intCol);
                                    }
                    ...
                    else if ( intNumColSel==1 && (intNumFilSel%intNumFilCop)==0 )
                    {
                        //Permitir pegar sólo si están seleccionadas varias filas (con la misma forma del área copiada) y una sola columna.
                        for (m=0; m<(intNumFilSel/intNumFilCop); m++)
                        {
                            stkDat=new java.util.StringTokenizer(strDat,"\n",false);
                            for (i=0; stkDat.hasMoreTokens(); i++)
                            {
                                ...
                                for (j=0; j<strFil.length; j++)
                                {
                                    intFil=i + intFilSel[0] + m*intNumFilCop;
                                    intCol=j + intColSel[0];
                                    if ( objTblMod.isCellEditable(intFil, intCol) && isValPegVal(strFil[j], intFil, intCol) )
                                    {
                                        objTblMod.setValueAt(strFil[j], intFil, intCol);
                    ...
                    else if ( (intNumFilSel%intNumFilCop)==0 && (intNumColSel%intNumColCop)==0 )
                    {
                        //Permitir pegar sólo si el área de pegado tiene la misma forma que el área de copiado.
                        for (m=0; m<(intNumFilSel/intNumFilCop); m++)
                        {
                            for (n=0; n<(intNumColSel/intNumColCop); n++)
                            {
                    ...
                    {
                        //No permitir pegar porque el área de copiado no coincide con el área de pegado.
                        javax.swing.JOptionPane.showMessageDialog(javax.swing.JOptionPane.getFrameForComponent(tblDat), "<HTML>No se pueden pegar los datos porque el área de copiado y pegado tienen formas diferentes.<BR>Asegúrese que el área de copiado y pegado tienen la misma forma y vuelva a intentarlo.<BR>Puede probar lo siguiente:<UL><LI>Haga clic en una única celda y luego elija la opción pegar.<LI>Seleccione un área de pegado que tenga la misma forma del área copiada.</UL></HTML>", "Mensaje del sistema Zafiro", javax.swing.JOptionPane.INFORMATION_MESSAGE);
                    }
                }
                //Liberar recursos.
                strDat=null;
    ...
    /**
     * Esta función copia las celdas seleccionadas al portapapeles presentando un cuadro de diálogo para que
     * el usuario decida lo que desea copiar.
     */
    private void mnuCopAvaActionPerformed(java.awt.event.ActionEvent evt)
    {
        int intFilSel[], intColSel[], i, j;
        Object objAux;
        try
        {
            intOpcSelMnu=INT_MNU_COP_AVA;
            //Permitir de manera predeterminada el click.
            blnCanCli=false;
            //Generar evento "beforeClick()".
            fireTblPopMnuListener(new ZafTblPopMnuEvent(this), INT_BEF_CLI);
            ...
            if (!blnCanCli)
            {
                StringBuffer stb=new StringBuffer();
                seleccionarCel();
                objCopAva.setVisible(true);
                if (objCopAva.getOpcSelDlg()==1)
                {
                    intFilSel=tblDat.getSelectedRows();
                    intColSel=objCopAva.getColCop();
                    //Preguntar si también se desea copiar la cabecera del JTable al Portapapeles.
                    if (objCopAva.isSelCopCabTbl())
                    {
                        //Obtener la cabecera que se va a enviar al Portapapeles,
                        for (j=0; j<intColSel.length; j++)
                        {
    ...
    /**
     * Esta función marca las casillas de verificación correspondientes a las filas seleccionadas
     * que se encuentran en la columna donde el usuario dió click con el botón derecho del mouse.
     */
    private void mnuMarChkCelSelActionPerformed(java.awt.event.ActionEvent evt)
    {
        int intFilSel[], i;
        intOpcSelMnu=INT_MNU_MAR_CHK_CEL_SEL;
        //Permitir de manera predeterminada el click.
        blnCanCli=false;
        //Generar evento "beforeClick()".
        fireTblPopMnuListener(new ZafTblPopMnuEvent(this), INT_BEF_CLI);
        ...
        if (!blnCanCli)
        {
            ...
            for (i=0; i<intFilSel.length; i++)
            {
                if (objTblMod.isColumnaEditable(intColTblCliButDer))
                    objTblMod.setChecked(true, intFilSel[i], intColTblCliButDer);
            }
            //Liberar recursos.
            intFilSel=null;
            //Generar evento "afterClick()".
    ...
    /**
     * Esta función desmarca las casillas de verificación correspondientes a las filas seleccionadas
     * que se encuentran en la columna donde el usuario dió click con el botón derecho del mouse.
     */
    private void mnuDesChkCelSelActionPerformed(java.awt.event.ActionEvent evt)
    {
        int intFilSel[], i;
        intOpcSelMnu=INT_MNU_DES_CHK_CEL_SEL;
        //Permitir de manera predeterminada el click.
        blnCanCli=false;
        //Generar evento "beforeClick()".
        fireTblPopMnuListener(new ZafTblPopMnuEvent(this), INT_BEF_CLI);
        ...
        if (!blnCanCli)
        {
            ...
            {
                if (objTblMod.isColumnaEditable(intColTblCliButDer))
                    objTblMod.setChecked(false, intFilSel[i], intColTblCliButDer);
            }
            //Liberar recursos.
            intFilSel=null;
            //Generar evento "afterClick()".
    ...
    private void mnuBorConActionPerformed(java.awt.event.ActionEvent evt)
    {
        ...
        if (!blnCanCli)
        {
            ...
            for (i=0; i<intFilSel.length; i++)
            {
                for (j=0; j<intColSel.length; j++)
                {
                    if (objTblMod.isCellEditable(intFilSel[i], intColSel[j]))
                    {
                        objTblMod.setValueAt(null, intFilSel[i], intColSel[j]);
                    }
                }
    ...
    /**
     * Esta función determina si la opción "Marcar casillas de verificación" está habilitada.
     * @return true: Si la opción "Marcar casillas de verificación" está habilitada.
     * <BR>false: En el caso contrario.
     */
    public boolean isMarcarCasillasVerificacionEnabled()
    {
        return mnuMarChkCelSel.isEnabled();
    }

    /**
     * Esta función establece si se debe habilitar la opción "Marcar casillas de verificación".
     * @param habilitado Valor booleano que determina si se debe habilitar la opción "Marcar casillas de verificación".
     */
    public void setMarcarCasillasVerificacionEnabled(boolean habilitado)
    {
        mnuMarChkCelSel.setEnabled(habilitado);
    }

    /**
     * Esta función determina si la opción "Desmarcar casillas de verificación" está visible.
     * @return true: Si la opción "Desmarcar casillas de verificación" está visible.
     * <BR>false: En el caso contrario.
     */
    public boolean isMarcarCasillasVerificacionVisible()
    {
        return mnuMarChkCelSel.isVisible();
    }

    /**
     * Esta función establece si se debe hacer visible la opción "Desmarcar casillas de verificación".
     * @param visible Valor booleano que determina si se debe hacer visible la opción "Desmarcar casillas de verificación".
     */
    public void setMarcarCasillasVerificacionVisible(boolean visible)
    {
        mnuMarChkCelSel.setVisible(visible);
    }

    /**
     * Esta función determina si la opción "Desmarcar casillas de verificación" está habilitada.
     * @return true: Si la opción "Desmarcar casillas de verificación" está habilitada.
     * <BR>false: En el caso contrario.
     */
    public boolean isDesmarcarCasillasVerificacionEnabled()
    {
        return mnuDesChkCelSel.isEnabled();
    }

    /**
     * Esta función establece si se debe habilitar la opción "Desmarcar casillas de verificación".
     * @param habilitado Valor booleano que determina si se debe habilitar la opción "Desmarcar casillas de verificación".
     */
    public void setDesmarcarCasillasVerificacionEnabled(boolean habilitado)
    {
        mnuDesChkCelSel.setEnabled(habilitado);
    }
     ...
     * <BR>false: En el caso contrario.
     */
    public boolean isDesmarcarCasillasVerificacionVisible()
    {
        return mnuDesChkCelSel.isVisible();
    }
     ...
     * @param visible Valor booleano que determina si se debe hacer visible la opción "Desmarcar casillas de verificación".
     */
    public void setDesmarcarCasillasVerificacionVisible(boolean visible)
    {
        mnuDesChkCelSel.setVisible(visible);
    }
    ...
    /**
     * Esta función determina si el valor a pegar es válido. Para determinar si el valor es válido
     * se basa en la configuración de la columna que está almacenada en el ZafTblMod. Es allí donde
     * está almacenado si la columna admite: cadenas, números enteros, números decimales o fechas.
     * @return true: Si el valor a pegar es válido.
     * <BR>false: En el caso contrario.
     */
    private boolean isValPegVal(String strConCel, int intFil, int intCol)
    {
        int intConCel;
        double dblConCel;
        boolean blnRes=true;
                ...
                case 1: //Columna que almacena números enteros. 
                    ...
                    else
                    {
                        try
                        {
                            ...
                            intConCel=Integer.parseInt(strConCel);
                            ...
                            if (objTblMod.getColumnDataTypeMin(intCol)!=null)
                            {
                                if (objTblMod.getColumnDataTypeMax(intCol)!=null)
                                {
                                    ...
                                    if (intConCel>=objTblMod.getColumnDataTypeMin(intCol).intValue() && intConCel<=objTblMod.getColumnDataTypeMax(intCol).intValue())
                                        blnRes=true;
                                    else
                                    {
                                        blnRes=false;
                                ...
                                else
                                {
                                    ...
                                    if (intConCel>=objTblMod.getColumnDataTypeMin(intCol).intValue())
                                    {
                                        blnRes=true;
                                    }
                                    else
                            ...
                            else
                            {
                                if (objTblMod.getColumnDataTypeMax(intCol)!=null)
                                {
                                    ...
                                    if (intConCel<=objTblMod.getColumnDataTypeMax(intCol).intValue())
                                    {
                                        blnRes=true;
                                    }
                                    else
                ...
                case 2: //Columna que almacena números reales.
                    ...
                    else
                    {
                        try
                        {
                            ...
                            java.math.BigDecimal objBigDec=new java.math.BigDecimal(strConCel);
                            ...
                            if (objTblMod.getColumnDataTypeMin(intCol)!=null)
                            {
                                ...
                                {
                                    //Validar que el valor sea >=Min y <=Max.
                                    if (Double.parseDouble(strConCel)>=objTblMod.getColumnDataTypeMin(intCol).doubleValue() && Double.parseDouble(strConCel)<=objTblMod.getColumnDataTypeMax(intCol).doubleValue())
                                    {
                                        blnRes=true;
                                    }
                                    else
                                ...
                                {
                                    //Validar que el valor sea >=Min.
                                    if (Double.parseDouble(strConCel)>=objTblMod.getColumnDataTypeMin(intCol).doubleValue())
                                    {
                                        blnRes=true;
                                    }
                                    else
                            ...
                            else
                            {
                                ...
                                {
                                    //Validar que el valor sea <=Max.
                                    if (Double.parseDouble(strConCel)<=objTblMod.getColumnDataTypeMax(intCol).doubleValue())
                                    {
                                        blnRes=true;
                                    }
                                    else
    ...
    /**
     * Esta función determina si se dió click en "Marcar casillas de verificación".
     * @return true: Si se dió click en "Marcar casillas de verificación".
     * <BR>false: En el caso contrario.
     */
    public boolean isClickMarcarCasillasVerificacion()
    {
        if (intOpcSelMnu==INT_MNU_MAR_CHK_CEL_SEL)
            return true;
        else
            return false;
    }
    ...
    /**
     * Esta función determina si se dió click en "Desmarcar casillas de verificación".
     * @return true: Si se dió click en "Desmarcar casillas de verificación".
     * <BR>false: En el caso contrario.
     */
    public boolean isClickDesmarcarCasillasVerificacion()
    {
        if (intOpcSelMnu==INT_MNU_DES_CHK_CEL_SEL)
            return true;
        else
            return false;
    }
    ...
    /**
     * Esta función determina si la opción "Copia avanzada" está habilitada.
     * @return true: Si la opción "Copia avanzada" está habilitada.
     * <BR>false: En el caso contrario.
     */
    public boolean isCopiaAvanzadaEnabled()
    {
        return mnuCopAva.isEnabled();
    }
    
    /**
     * Esta función establece si se debe habilitar la opción "Copia avanzada".
     * @param habilitado Valor booleano que determina si se debe habilitar la opción "Copia avanzada".
     */
    public void setCopiaAvanzadaEnabled(boolean habilitado)
    {
        mnuCopAva.setEnabled(habilitado);
    }
    
    /**
     * Esta función determina si la opción "Copia avanzada" está visible.
     * @return true: Si la opción "Copia avanzada" está visible.
     * <BR>false: En el caso contrario.
     */
    public boolean isCopiaAvanzadaVisible()
    {
        return mnuCopAva.isVisible();
    }
    
    /**
     * Esta función establece si se debe hacer visible la opción "Copia avanzada".
     * @param visible Valor booleano que determina si se debe hacer visible la opción "Copia avanzada".
     */
    public void setCopiaAvanzadaVisible(boolean visible)
    {
        mnuCopAva.setVisible(visible);
    }
    
    /**
     * Esta función obtiene el número total de filas que se insertaron la última vez que se dió click en el menú "Insertar fila" o "Insertar filas...".
     * @return El número total de filas insertadas.
     */
    public int getNumeroTotalFilasInsertadas()
    {
        return intNumTotFilIns;
    }
    
    /**
     * Esta función obtiene un arreglo con el listado de las filas que se eliminaron la última vez que se dió click en el menú "Eliminar fila(s)".
     * @return El arreglo con las filas eliminadas.
     */
