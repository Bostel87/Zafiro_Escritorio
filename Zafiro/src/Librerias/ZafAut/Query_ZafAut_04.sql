...
/*QUERY PARA OBTENER LAS RETENCIONES PENDIENTES.*/
...
WHERE a1.co_emp=1 AND a1.co_cli=(SELECT co_cli FROM tbm_cabCotVen WHERE co_emp=1 AND co_loc=1 AND co_cot=24455) 
AND a1.st_reg IN ('A','R','C','F') AND a2.st_reg IN ('A','C') AND (a2.mo_pag+a2.nd_abo)<0 AND a2.nd_porRet>0
