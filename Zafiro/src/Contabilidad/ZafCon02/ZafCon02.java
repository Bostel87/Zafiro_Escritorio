 ...
 * ZafCon02.java
 *
 * Created on 20 de septiembre de 2005, 14:02 PM
 */
package Contabilidad.ZafCon02;
 ...
 * @author  Eddye Lino
 */
public class ZafCon02 extends javax.swing.JInternalFrame
{
    //Constantes
    ...
    private int INT_VEC_EST_CON    = 8; //7) Referencia: Se debe asignar una cadena vacía o null
     
    //ArrayList: Documentos Asociados
    private java.util.ArrayList arlRegIsDocAso, arlDatIsDocAso;
    final int INT_ARL_COD_EMP_ASO=0;
    final int INT_ARL_COD_LOC_ASO=1;
    final int INT_ARL_COD_TIP_DOC_ASO=2;
    final int INT_ARL_COD_DIA_ASO=3;
    final int INT_ARL_NUM_DOC_ASO=4;
    final int INT_ARL_DES_COR_TIP_DOC_ASO=5;
    final int INT_ARL_DES_LAR_TIP_DOC_ASO=6;  
    ...
    private String strFecDocIni, strEstImpDoc;
    private String strNumDia_Glo;
    private String strCtaPadExi;//si proviene desde el programa de otros movimientos bancarios pero de grupo.
    private String strSQL, strAux;
     
    private String strVer=" v0.15.11 ";
    
    /** Crea una nueva instancia de la clase ZafCon02. */
    public ZafCon02(ZafParSis obj){

        try{
            strCtaPadExi="";
            objParSis=(ZafParSis)obj.clone();
            arlDatIsDocAso=new java.util.ArrayList();
    ...
    public ZafCon02(ZafParSis obj, Integer codigoCuenta){
        try{
            strCtaPadExi="";
            objParSis=(ZafParSis)obj.clone();
    ...
    /** Crea una nueva instancia de la clase ZafCon02. */
    public ZafCon02(ZafParSis obj, int codigoEmpresa, int codigoLocal, int codigoTipoDocumento, int codigoDocumento, int codigoMenu)
    {
        try{
            objParSis=(ZafParSis)obj.clone();
            objUti=new ZafUtil();
    ...
    public ZafCon02(ZafParSis obj, int codigoEmpresaBancaria, String existePadreGrupo, String codigoCuenta, String numeroCuenta, String nombreCuenta, BigDecimal valorTransferencia){
        try{
            objParSis=(ZafParSis)obj.clone();
    ...
    private boolean configurarFrm()
    {
        ...
        try
        {
            ...
            if(objParSis.getCodigoMenu()==1708){
                ...
                panCab.remove(this.panCabCieCaj);
            }
    ...
    private void initComponents() {
        ...
        panCabCieCaj.setLayout(null);

        lblValDoc.setText("Valor del documento:");
        lblValDoc.setToolTipText("Número de diario");
        panCabCieCaj.add(lblValDoc);
        lblValDoc.setBounds(450, 0, 130, 20);
        ...
        panCabCieCaj.add(txtValDoc);
        txtValDoc.setBounds(578, 0, 100, 20);

        panCab.add(panCabCieCaj, java.awt.BorderLayout.CENTER);
    ...
    private boolean generarDiarioLocal(){
        ...
        try{
            if(objParSis.getCodigoMenu()==1708){
                objAsiDia.generarDiario(txtCodTipDoc.getText(), txtValDoc.getText(), txtValDoc.getText());
            }
        }
        catch(Exception e){
            objUti.mostrarMsgErr_F1(this, e);
    ...
    private class MiToolBar extends ZafToolBar
    {
        ...
        public boolean beforeAnular(){
            boolean blnRes=true;
            if(objParSis.getCodigoMenu()==1708){
                if(! isDocumentosAsociados()){
                    if (  (!txtCodTipDoc.getText().equals("30")) && (!txtCodTipDoc.getText().equals("86"))  && (!txtCodTipDoc.getText().equals("87")) &&  (!txtCodTipDoc.getText().equals("88")) &&  (!txtCodTipDoc.getText().equals("89")) 
                           &&  (!txtCodTipDoc.getText().equals("113")) &&  (!txtCodTipDoc.getText().equals("92"))   &&  (!txtCodTipDoc.getText().equals("184")) &&  (!txtCodTipDoc.getText().equals("288"))  &&  (!txtCodTipDoc.getText().equals("194"))   )
                    {
                        mostrarMsgInf("Sólo es posible anular documentos del tipo 30=DIGECO, 86=TRBALO, 87=TRBAEX, 88=TRBADA, 89=TRBAPD, 113=TRBAVA, 92=CIECAJ, 184=CIECA2, 288=CIECA3 ");
                        blnRes=false;
                    }
                    strAux=objTooBar.getEstadoRegistro();
                    if (strAux.equals("Anulado")){
                    ...
                    mostrarMsgInf("El " + txtDesCorTipDoc.getText() + " tiene documentos asociados.\n");
                    for(int i=0; i<arlDatIsDocAso.size(); i++){
                        mostrarMsgInf(" " + objUti.getStringValueAt(arlDatIsDocAso, i, INT_ARL_DES_COR_TIP_DOC_ASO) + "    -    "  +  objUti.getStringValueAt(arlDatIsDocAso, i, INT_ARL_DES_LAR_TIP_DOC_ASO)  +  "     #     " + objUti.getStringValueAt(arlDatIsDocAso, i, INT_ARL_NUM_DOC_ASO) + "\n" );
                    }
                    mostrarMsgInf("No es posible anular el documento.");
                    blnRes=false;
                }
            }
            else{
    ...
    private boolean insertarReg()
    {
        ...
        try
        {
            ...
            if (con!=null){
                if(objParSis.getCodigoMenu()==1708){
                    if( ! (   (txtValDoc.getText().toString().equals("")) || (txtValDoc.getText().toString().length()<=0) )  ){
                        if (objAsiDia.insertarDiario(con, objParSis.getCodigoEmpresa(), objParSis.getCodigoLocal(), Integer.parseInt(txtCodTipDoc.getText()), txtNumDia.getText(), objUti.parseDate(dtpFecDoc.getText(),"dd/MM/yyyy"), "O")){
                        ...
                        mostrarMsgInf("<HTML>El campo valor del documento es obligatorio.<BR>Ingrese el valor del documento y vuelva a intentarlo.</HTML>");
                }
                else{
                    if(strCtaPadExi.equals("S")){
                        ...
                        {   blnExiCtasAsiDia = validaExiCtaDetAsiDia(con, objParSis.getCodigoEmpresa(), objParSis.getCodigoLocal(), Integer.parseInt(txtCodTipDoc.getText()), strNumDia_Glo);
                            
                            if (blnExiCtasAsiDia)
                            {  
                                if (actualizaNumUltDocCabTipDoc(con))
                                {  txtCodDoc.setText("" + objAsiDia.getCodigoDiario());
                                   con.commit();
                                   blnRes=true;
                                }
    ...
    private boolean validaExiCtaDetAsiDia(Connection conexion, int intCodEmp, int intCodLoc, int intCodTipDoc, String strNumDia)
    {
        boolean blnRes = false;
        try
        {
            stm = conexion.createStatement();
            strSQL =" SELECT count(a.*) as nd_CanReg FROM tbm_detdia as a";
            strSQL+=" INNER JOIN (  ";
            strSQL+="      SELECT co_emp, co_loc, co_tipdoc, co_dia, tx_numdia FROM tbm_cabdia ";
            strSQL+="      WHERE st_reg = 'A' ";
            strSQL+="      AND co_emp = " + intCodEmp;
            strSQL+="      AND co_loc = " + intCodLoc;
            strSQL+="      AND co_tipdoc = " + intCodTipDoc;
    ...
    private boolean actualizaNumUltDocCabTipDoc(Connection conexion)
    {
        boolean blnRes = true;
        Statement stmLoc;
        try
        {
           stmLoc = conexion.createStatement();
    ...
    /**
     * Función que permite saber si el documento tiene documentos asociados.
     * @return 
     */
    private boolean isDocumentosAsociados(){
        boolean blnRes=false;
        Connection conLoc;
        Statement stmLoc;
        ResultSet rstLoc;
        arlDatIsDocAso.clear();
        try{
            conLoc=DriverManager.getConnection(objParSis.getStringConexion(), objParSis.getUsuarioBaseDatos(), objParSis.getClaveBaseDatos());
            if(conLoc!=null){
                stmLoc=conLoc.createStatement();
                strAux="";
                strAux+=" SELECT a1.co_emp, a1.co_loc, a1.co_tipDoc, a1.co_dia, a2.co_locRec";
                strAux+="      , a2.co_tipDocRec, a2.co_docRec, a3.ne_numDoc1, a4.tx_desCor, a4.tx_desLar";
                strAux+="  FROM tbm_cabDia AS a1";
                strAux+="  INNER JOIN tbm_detRecEfe AS a2";
                strAux+="  ON a1.co_emp=a2.co_emp AND a1.co_loc=a2.co_locRec AND a1.co_tipDoc=a2.co_tipDocRec AND a1.co_dia=a2.co_docRec";
                strAux+="  INNER JOIN tbm_cabRecEfe AS a3";
                strAux+="  ON a2.co_emp=a3.co_emp AND a2.co_loc=a3.co_loc AND a2.co_tipDoc=a3.co_tipDoc AND a2.co_doc=a3.co_doc";
		strAux+="  INNER JOIN tbm_cabTipDoc AS a4";
		strAux+="  ON a3.co_emp=a4.co_emp AND a3.co_loc=a4.co_loc AND a3.co_tipDoc=a4.co_tipDoc";
                ...
                strAux+="  AND a1.co_dia=" + txtCodDoc.getText() + "";
                strAux+="  AND a3.st_reg NOT IN('E','I')";
                rstLoc=stmLoc.executeQuery(strAux);
                if(rstLoc.next()){
                    arlRegIsDocAso=new java.util.ArrayList();
                    arlRegIsDocAso.add(INT_ARL_COD_EMP_ASO,         "" + rstLoc.getString("co_emp"));
                    arlRegIsDocAso.add(INT_ARL_COD_LOC_ASO,         "" + rstLoc.getString("co_loc"));
                    arlRegIsDocAso.add(INT_ARL_COD_TIP_DOC_ASO,     "" + rstLoc.getString("co_tipDoc"));
                    arlRegIsDocAso.add(INT_ARL_COD_DIA_ASO,         "" + rstLoc.getString("co_dia"));
                    arlRegIsDocAso.add(INT_ARL_NUM_DOC_ASO,         "" + rstLoc.getString("ne_numDoc1"));
                    arlRegIsDocAso.add(INT_ARL_DES_COR_TIP_DOC_ASO, "" + rstLoc.getString("tx_desCor"));
                    arlRegIsDocAso.add(INT_ARL_DES_LAR_TIP_DOC_ASO, "" + rstLoc.getString("tx_desLar"));
                    
                    arlDatIsDocAso.add(arlRegIsDocAso);
                    blnRes=true;
                }
                rstLoc.close();
                rstLoc=null;
