 ...
 * @author  Eddye Lino
 */
public class ZafTblPopMnuIncDec extends javax.swing.JPopupMenu
{
    //Constantes:
    ...
    private static final int INT_MNU_SEL_FIL=3;         /**Menú en el que se dió click: Indica "Seleccionar fila".*/
    private static final int INT_MNU_SEL_COL=4;         /**Menú en el que se dió click: Indica "Seleccionar columna".*/
    private static final int INT_MNU_SEL_TOD=5;         /**Menú en el que se dió click: Indica "Seleccionar todo".*/
    private static final int INT_MNU_INS_FIL=6;         /**Menú en el que se dió click: Indica "Insertar fila".*/
    private static final int INT_MNU_INS_VAR_FIL=7;     /**Menú en el que se dió click: Indica "Insertar filas".*/
    private static final int INT_MNU_ELI_FIL=8;         /**Menú en el que se dió click: Indica "Eliminar fila(s)".*/
    private static final int INT_MNU_BOR_CON=9;         /**Menú en el que se dió click: Indica "Borrar contenido".*/
    private static final int INT_MNU_OCU_COL=10;        /**Menú en el que se dió click: Indica "Ocultar columna".*/
    private static final int INT_MNU_MCO_TOD_COL=11;    /**Menú en el que se dió click: Indica "Mostrar columna: Todas las columnas".*/
    private static final int INT_MNU_AJU_ANC_COL=12;    /**Menú en el que se dió click: Indica "Ajustar ancho de columnas automáticamente".*/
    private static final int INT_MNU_TSE_CEL=13;        /**Menú en el que se dió click: Indica "Tipo de selección: Selección por celda".*/
    private static final int INT_MNU_TSE_FIL=14;        /**Menú en el que se dió click: Indica "Tipo de selección: Selección por fila".*/
    private static final int INT_MNU_TSE_COL=15;        /**Menú en el que se dió click: Indica "Tipo de selección: Selección por columna".*/
    private static final int INT_MNU_INC_DEC=16;        /**Menú en el que se dió click: Indica "Incremento/Decremento".*/
    ...
    private javax.swing.JMenuItem mnuIncDec;
    ...
    public ZafTblPopMnuIncDec(javax.swing.JTable tabla)
    {
        tblDat=tabla;
        blnCanCli=false;
        ...
        mnuIncDec=new javax.swing.JMenuItem("Establecer los porcentajes de incremento/decremento");
        mnuIncDec.setAccelerator(javax.swing.KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_X, java.awt.event.InputEvent.CTRL_MASK));
        mnuIncDec.setEnabled(false);
...
//            }
        ...
        mnuIncDec.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                mnuIncDecActionPerformed(evt);
            }
        });
...
//        System.out.println("LA LONGITUD ES: " + intFilSel.length);
        if(intFilSel.length>1)
            mnuIncDec.setEnabled(true);
        else
            mnuIncDec.setEnabled(false);
        intFilSel=null;
...
//            this.show(evt.getComponent(),evt.getX(),evt.getY());
    ...
    public int getNumFilSel(){
        int intFilSel[];
        intFilSel=tblDat.getSelectedRows();
        return (intFilSel.length);
    }
    ...
    private void mnuIncDecActionPerformed(java.awt.event.ActionEvent evt){
        intOpcSelMnu=INT_MNU_INC_DEC;
    ...
    public boolean isClickIncreDecre()
    {
        if (intOpcSelMnu==INT_MNU_INC_DEC)
            return true;
        else
            return false;
    }
