 ...
 * @author sistemas9
 */
public class ZafCon64_01 extends javax.swing.JInternalFrame 
{
    ...
    private ZafCon64 objCon64;
    private ZafCon64_02 objCon64_02;
    ...
    /** Crea una nueva instancia de la clase ZafCon64_01. */
    public ZafCon64_01(ZafParSis obj){
        try{
            intCodRegTmp=0;
            initComponents();
            //Inicializar objetos.
            ...
            objCon64= new ZafCon64(objParSis);
    ...
    /** Crea una nueva instancia de la clase ZafCon64_01. */
    public ZafCon64_01(ZafParSis obj, int codigoEmpresa, int codigoLocal, int codigoTipoDocumento, int codigoDocumento, int codigoMenu)
    {
        try
        {
            intCodRegTmp=0;
    ...
    private boolean configurarFrm()
    {
        ...
        try
        {
            ...
            this.setTitle(strAux + objCon64.strVer);
    ...
    private boolean configurarTblDat()
    {
        ...
        try
        {
            ...
            objTblCelEdiCmbBoxTrs.addItem("Egreso bancario");
            objTblCelEdiCmbBoxTrs.addItem("Ingreso bancario");
                ...
                public void afterEdit(Librerias.ZafTblUti.ZafTblEvt.ZafTableEvent evt) {
                    if( (objTblCelEdiCmbBoxTrs.getSelectedIndex()==0) || (objTblCelEdiCmbBoxTrs.getSelectedIndex()==1)   ){
                        if(objTblCelEdiCmbBoxTrs.getSelectedIndex()==0)
                            objTblMod.setValueAt("E", tblDat.getSelectedRow(), INT_TBL_DAT_OPC_TRS);
                        if(objTblCelEdiCmbBoxTrs.getSelectedIndex()==1)
                            objTblMod.setValueAt("I", tblDat.getSelectedRow(), INT_TBL_DAT_OPC_TRS);
    ...
    private boolean mostrarVentanaDocumentosAsociados(int fila){
        ...
        try{
            ...
            objCon64_02=new ZafCon64_02(javax.swing.JOptionPane.getFrameForComponent(this), true, objParSis,intCodEmpRel, intCodLocRel, intCodTipDocRel, intCodDocRel, intCodRegDet, objTooBar.getEstado());
            objCon64_02.setVisible(true);
        }
        catch(Exception e){
            objUti.mostrarMsgErr_F1(this, e);
            blnRes=false;
    ...
    private boolean isCamVal(){
        ...
        if( ! validaIngresosEgresos()){
            tabFrm.setSelectedIndex(0);
            mostrarMsgInf("<HTML>La suma de egresos es diferente a la suma de los ingresos.<BR>Puede ser también que tanto el ingreso como el egreso estén dentro de la misma empresa, <BR>si éste fuera el caso, esa transacción deberá realizarla por la empresa y no por grupo.<BR>Verifique los datos ingresados y vuelva a intentarlo.</HTML>");
            return false;
        }
    ...
    /**Función que valida que la suma de ingresos y egresos esté cuadrada
     * @return true: Si cuadran las sumatorias
     * <BR> false: En caso contrario
     */
    private boolean validaIngresosEgresos(){
        boolean blnRes=true;
        String strOpcTrsIni="", strCuaIni="";
        BigDecimal bdeValFilIni=new BigDecimal("0");
        String strOpcTrsFin="", strCuaFin="";
        BigDecimal bdeValFilFin=new BigDecimal("0");
        int intReg=1;
        int intCodEmpRegIni=-1, intCodEmpRegFin=-1;
        String strCodEmpUno="", strCodEmpDos="";
        try{

            if(objTblMod.getRowCountTrue()==2){
                strCodEmpUno=objTblMod.getValueAt(0, INT_TBL_DAT_COD_EMP).toString();
                strCodEmpDos=objTblMod.getValueAt(1, INT_TBL_DAT_COD_EMP).toString();
            }
            
            if(  (objTblMod.getRowCountTrue()==2)  && (strCodEmpUno.equals(strCodEmpDos)) ){
                if(objTblMod.getValueAt(0, INT_TBL_DAT_COD_EMP).toString().equals(objTblMod.getValueAt(1, INT_TBL_DAT_COD_EMP))){
                    for(int i=0; i<objTblMod.getRowCountTrue(); i++){
                        objTblMod.setValueAt("", i, INT_TBL_DAT_CUA);
                    }
                    for(int i=0; i<objTblMod.getRowCountTrue(); i++){

                        strOpcTrsIni=objTblMod.getValueAt(i, INT_TBL_DAT_OPC_TRS)==null?"":objTblMod.getValueAt(i, INT_TBL_DAT_OPC_TRS).toString();
                        bdeValFilIni=new BigDecimal(objTblMod.getValueAt(i, INT_TBL_DAT_VAL_TRS)==null?"0":objTblMod.getValueAt(i, INT_TBL_DAT_VAL_TRS).toString());
                        strCuaIni=objTblMod.getValueAt(i, INT_TBL_DAT_CUA)==null?"":objTblMod.getValueAt(i, INT_TBL_DAT_CUA).toString();
                        intCodEmpRegIni=Integer.parseInt(objTblMod.getValueAt(i, INT_TBL_DAT_COD_EMP)==null?"0":objTblMod.getValueAt(i, INT_TBL_DAT_COD_EMP).toString());

                        if(strCuaIni.equals("")){
                            if(strOpcTrsIni.equals("I")){
                                for(int j=0; j<objTblMod.getRowCountTrue(); j++){
                                    strOpcTrsFin=objTblMod.getValueAt(j, INT_TBL_DAT_OPC_TRS)==null?"":objTblMod.getValueAt(j, INT_TBL_DAT_OPC_TRS).toString();
                                    bdeValFilFin=new BigDecimal(objTblMod.getValueAt(j, INT_TBL_DAT_VAL_TRS)==null?"0":objTblMod.getValueAt(j, INT_TBL_DAT_VAL_TRS).toString());
                                    strCuaFin=objTblMod.getValueAt(j, INT_TBL_DAT_CUA)==null?"":objTblMod.getValueAt(j, INT_TBL_DAT_CUA).toString();
                                    intCodEmpRegFin=Integer.parseInt(objTblMod.getValueAt(j, INT_TBL_DAT_COD_EMP)==null?"0":objTblMod.getValueAt(j, INT_TBL_DAT_COD_EMP).toString());


                                    if(strCuaFin.equals("")){
                                        if(strOpcTrsFin.equals("E")){
                                            if(intCodEmpRegIni==intCodEmpRegFin){
                                                if(bdeValFilIni.compareTo(bdeValFilFin)==0){
                                                    objTblMod.setValueAt(""+ intReg, i, INT_TBL_DAT_CUA);
                                                    objTblMod.setValueAt(""+ intReg, j, INT_TBL_DAT_CUA);
                                                    objTblMod.setValueAt(""+ intCodEmpRegIni, j, INT_TBL_DAT_COD_EMP_REL);
                                                    objTblMod.setValueAt(""+ intCodEmpRegFin, i, INT_TBL_DAT_COD_EMP_REL);
                                                    intReg++;
                                                    break;
                                                }
                                            }
                    ...
                    for(int i=0; i<objTblMod.getRowCountTrue(); i++){
                        strOpcTrsIni=objTblMod.getValueAt(i, INT_TBL_DAT_OPC_TRS)==null?"":objTblMod.getValueAt(i, INT_TBL_DAT_OPC_TRS).toString();
                        strCuaIni=objTblMod.getValueAt(i, INT_TBL_DAT_CUA)==null?"":objTblMod.getValueAt(i, INT_TBL_DAT_CUA).toString();
                        if( (strOpcTrsIni.equals("I")) || (strOpcTrsIni.equals("E"))  ){
                            if(strCuaIni.equals("")){
                                blnRes=false;
                                break;
                            }
    ...
    private boolean consultarReg()
    {
        ...
        try
        {
            ...
            if (con!=null)
            {
                ...
                strSQL+=" WHERE a3.tx_tiptra IN ('E', 'I')";
        ...
        try
        {
            ...
            if( ! txtCodTipDoc.getText().equals("")){
                ...
                if (con!=null){
                    ...
                    strSQL+=" AND a1.co_tipDoc=" + objUti.getIntValueAt(arlDatCon, intIndiceCon, INT_ARL_CON_COD_TIPDOC);
                    strSQL+=" AND a1.co_doc=" + objUti.getIntValueAt(arlDatCon, intIndiceCon, INT_ARL_CON_COD_DOC);
                    strSQL+=" AND a2.tx_tiptra IN ('E', 'I')";
                    ...
                    while (rst.next()){
                        ...
                        if(strTipTrs.equals("E"))
                            vecReg.setElementAt("Egreso bancario", INT_TBL_DAT_TIP_TRS);
                        else if(strTipTrs.equals("I"))
                            vecReg.setElementAt("Ingreso bancario", INT_TBL_DAT_TIP_TRS);
    ...
    private boolean insertarDiario(){
        ...
        int intCodTipDia=-1;
        String strNumDia="", strFecDia="", strValDia="";
        ...
        try{
            if(con!=null){
                ...
                //si la tx es en la misma empresa EJm: Tuval(Pacifico) tx a Tuval(Pichincha)
                if(  (objTblMod.getRowCountTrue()==2)  && (strCodEmpUno.equals(strCodEmpDos))   )
                {
                    for(int i=0; i<objTblMod.getRowCountTrue(); i++) {
                        vecDatDetTrs.clear();
                        ...
                        if(  strOpcTrs.equals("E")  ){
                            ...
                            intCodCtaDeb=Integer.parseInt(objTblMod.getValueAt(fila, INT_TBL_DAT_COD_CTA)==null?"0":objTblMod.getValueAt(fila, INT_TBL_DAT_COD_CTA).toString());
                            intCodCtaHab=Integer.parseInt(objTblMod.getValueAt(i, INT_TBL_DAT_COD_CTA)==null?"0":objTblMod.getValueAt(i, INT_TBL_DAT_COD_CTA).toString());

                            objAsiDia.generarDiario(intCodEmp, intCodLoc, intCodTipDia, intCodCtaDeb,  strValDia, intCodCtaHab, strValDia);
                            objAsiDia.setGlosa("P/registrar transferencia entre cuentas desde " + getNomEmp(intCodEmp) + "(" + objTblMod.getValueAt(i, INT_TBL_DAT_NOM_CTA) +  ")" + " a " + getNomEmp(intCodEmpRel) + "(" + objTblMod.getValueAt((i+1), INT_TBL_DAT_NOM_CTA) + ")");
                            ...
                            if (objAsiDia.insertarDiario(con, intCodEmp, intCodLoc, intCodTipDia, ""+intCodDia, strNumDia, objUti.parseDate(strFecDia,"dd/MM/yyyy"), objParSis.getCodigoMenu())){
                                ...
                                if((strOpcTrs.equals("E")  )  ){
                                    ...
                                    strSQL+=", " + objTblMod.getValueAt(i, INT_TBL_DAT_VAL_TRS) + " AS nd_valtra";//nd_valtra
                                    //Cantidad en palabras.
                                    strSQL+=", " + objUti.codificar(getValorPalabras(objTblMod.getValueAt(i, INT_TBL_DAT_VAL_TRS).toString())) + " AS tx_valtrapal";//tx_valtrapal
                                    ...
                                    strSQL+=", Null AS nd_adv";//nd_adv
                                    strSQL+=", Null AS nd_fod";//nd_fod
                ...
                else{//si la tx es en diferentes empresas EJm: Tuval(Pacifico) tx a Castek(Pichincha)
                    for(int i=0; i<objTblMod.getRowCountTrue(); i++){
                        vecDatDetTrs.clear();
                        objAsiDia.inicializar();
                        ...
                        strOpcTrs=objTblMod.getValueAt(i, INT_TBL_DAT_OPC_TRS)==null?"":objTblMod.getValueAt(i, INT_TBL_DAT_OPC_TRS).toString();
                        if(  (strOpcTrs.equals("I"))  || (strOpcTrs.equals("E"))  ){
                            intCodTipDia=86;
                        }
                        ...
                        if(strTipTrs.equals("I")){
                            intCodCtaDeb=Integer.parseInt(objTblMod.getValueAt(i, INT_TBL_DAT_COD_CTA)==null?"0":objTblMod.getValueAt(i, INT_TBL_DAT_COD_CTA).toString());
                            intCodCtaHab=getCuentaHaber(intCodEmp, intCodEmpRel);

                            objAsiDia.generarDiario(intCodEmp, intCodLoc, intCodTipDia, intCodCtaDeb,  strValDia, intCodCtaHab, strValDia);
                            objAsiDia.setGlosa("P/registrar transferencia entre cuentas desde " + getNomEmp(intCodEmpRel) + " a " + getNomEmp(intCodEmp));
                        }
                        else if(strTipTrs.equals("E")){
                            intCodCtaDeb=getCuentaDebe(intCodEmp, intCodEmpRel);
                            intCodCtaHab=Integer.parseInt(objTblMod.getValueAt(i, INT_TBL_DAT_COD_CTA)==null?"0":objTblMod.getValueAt(i, INT_TBL_DAT_COD_CTA).toString());

                            objAsiDia.generarDiario(intCodEmp, intCodLoc, intCodTipDia, intCodCtaDeb,  strValDia, intCodCtaHab, strValDia);
                            objAsiDia.setGlosa("P/registrar transferencia entre cuentas desde " + getNomEmp(intCodEmp) + " a " + getNomEmp(intCodEmpRel));
                        }
    ...
    private int getCuentaHaber(int codigoEmpresa, int codigoEmpresaRelacionada)
    {
        int intCtaHab=-1;
        try
        {
            ...
            {
                case 1:
                    switch(codigoEmpresaRelacionada)
                    {
                        case 2:
                            intCtaHab=4385;
                            break;
    ...
    private int getCuentaDebe(int codigoEmpresa, int codigoEmpresaRelacionada)
    {
            int intCtaDeb=-1;
            try
            {
                            ...
                            for(int k=0;k<objTblMod.getRowCountTrue();k++){
                                ...
                                {   
                                    case 276: 
                                        ...
                                        strSQL+="      , a4.co_usring, a6.tx_nom AS tx_nomUsrIng, a4.co_usrMod, a6.tx_nom AS tx_nomUsrMod, a4.st_Reg, a4.st_imp, a5.tx_Descor, a5.tx_Deslar ";
                                        strSQL+=" FROM tbm_cabotrmovban AS a1 ";
                                        ...
                                        strSQL+=" INNER JOIN tbm_usr as a6 ON a4.co_usrMod=a6.co_usr and a4.co_usring=a6.co_usr ";
                                        strSQL+=" WHERE a1.co_emp=" + objParSis.getCodigoEmpresa() + "";
                                        strSQL+=" AND a1.co_loc=" + objParSis.getCodigoLocal() + "";
                                        strSQL+=" AND a1.co_tipDoc=" + txtCodTipDoc.getText() + "";
                                        strSQL+=" AND a1.co_doc=" + txtCodDoc.getText() + "";
                                    ...
                                    case 383: 
                                        //<editor-fold defaultstate="collapsed" desc="/* Carta transferencias locales... */"> 
                                        if(strTipTrs.equals("Egreso bancario")){
                                            stmRpt=conRpt.createStatement();
                                            strSQL ="";
                                            strSQL+=" SELECT a2.co_emp, a2.co_loc, a2.co_tipdoc, a2.co_doc, a2.co_reg, a2.tx_tiptra, a2.co_emptra, c1.tx_nom AS tx_nomEmp";
                                            strSQL+="      , a2.co_ctatra, round(a2.nd_valtra,2) AS nd_valtraBan, a2.st_regrep, a3.co_cta, a3.tx_codCta, a3.tx_desLar AS tx_nomCta";
                                            strSQL+="      , a4.tx_nom AS tx_nomEmp, a2.co_reg, b3.fe_car, b3.co_ctabanegr";
                                            strSQL+="      , b3.co_ctabaning, round(b3.nd_valtra,2) AS nd_valtra, b3.tx_valtrapal, CASE WHEN b3.st_mosparresp IS NULL THEN '' ELSE b3.st_mosparresp END AS st_mosparresp, b3.tx_mottra";
                                            strSQL+="      , b3.tx_ejeasi, b3.st_fircon, b3.tx_firaut1, b3.tx_carfiraut1, b3.tx_firaut2, b3.tx_carfiraut2";
                                            strSQL+="      , b4.co_ban AS co_banIng, b4.tx_numctaban AS tx_numctabanIng, b5.tx_desCor AS tx_desCorBcoIng, b5.tx_desLar AS tx_desLarBcoIng";
                                            strSQL+="      , b6.co_ban AS co_banEgr, b6.tx_numctaban AS tx_numctabanEgr, b7.tx_desCor AS tx_desCorBcoEgr, b7.tx_desLar AS tx_desLarBcoEgr";
                                            strSQL+="      , b4.co_ctaprvloc, c2.tx_nom AS tx_nomPrvLoc, CASE WHEN b6.tx_tipctaban='C' THEN 'cta. cte.' ELSE 'Cuenta de Ahorros' END AS tx_tipctabanEgr";
                                            strSQL+="      , CASE WHEN b4.tx_tipctaban='C' THEN 'cta.cte.' ELSE 'cta.aho.' END AS tx_tipctabanIng";
                                            strSQL+=" FROM tbm_cabOtrMovBan AS a1 INNER JOIN";
                                            strSQL+=" ( (  tbm_detOtrMovBan AS a2 INNER JOIN tbm_emp AS c1  ON a2.co_empTra=c1.co_emp)";
                                            strSQL+="    LEFT OUTER JOIN tbm_docgenotrmovban AS b2 ON a2.co_emp=b2.co_emp AND a2.co_loc=b2.co_loc AND a2.co_tipDoc=b2.co_tipDoc AND a2.co_doc=b2.co_doc AND a2.co_reg=b2.co_reg";
                                            strSQL+="    LEFT OUTER JOIN ( ( tbm_cartrabandia AS b3";
                                            strSQL+="                        INNER JOIN (tbm_ctaban AS b4 LEFT OUTER JOIN tbm_cli AS c2 ON b4.co_emp=c2.co_emp AND b4.co_ctaprvloc=c2.co_cli)";
                                            strSQL+="                        ON b3.co_emp=b4.co_emp AND b3.co_ctaBanIng=b4.co_ctaban";
                                            strSQL+="                        LEFT OUTER JOIN tbm_var AS b5 ON b4.co_ban=b5.co_reg";
                                            strSQL+="                      ) ";
                                            strSQL+="                      INNER JOIN tbm_ctaban AS b6 ON b3.co_emp=b6.co_emp AND b3.co_ctaBanEgr=b6.co_ctaban";
                                            strSQL+="                      LEFT OUTER JOIN tbm_var AS b7 ON b6.co_ban=b7.co_reg";
                                            strSQL+="    )  ON b2.co_empRelCabDia=b3.co_emp AND b2.co_locRelCabDia=b3.co_loc AND b2.co_tipDocRelCabDia=b3.co_tipDoc AND b2.co_docRelCabDia=b3.co_dia";
                                            strSQL+=" )  ON a1.co_emp=a2.co_emp AND a1.co_loc=a2.co_loc AND a1.co_tipDoc=a2.co_tipDoc AND a1.co_doc=a2.co_doc";
                                            strSQL+=" INNER JOIN tbm_plaCta AS a3 ON a2.co_empTra=a3.co_emp AND a2.co_ctaTra=a3.co_cta";
                                            strSQL+=" INNER JOIN tbm_emp AS a4 ON a2.co_empTra=a4.co_emp";
                                            strSQL+=" WHERE a2.tx_tipTra='E'";
                                            strSQL+=" AND a1.co_emp=" + objUti.getIntValueAt(arlDatCon, intIndiceCon, INT_ARL_CON_COD_EMP);
                                            strSQL+=" AND a1.co_loc=" + objUti.getIntValueAt(arlDatCon, intIndiceCon, INT_ARL_CON_COD_LOC);
                                            strSQL+=" AND a1.co_tipDoc=" + objUti.getIntValueAt(arlDatCon, intIndiceCon, INT_ARL_CON_COD_TIPDOC);
                                            strSQL+=" AND a1.co_doc=" + objUti.getIntValueAt(arlDatCon, intIndiceCon, INT_ARL_CON_COD_DOC);
                                            ...
                                            stmRpt=conRpt.createStatement();
                                            strFecCar =" SELECT a2.co_emp, a2.co_loc, a2.co_tipdoc, a2.co_doc, a2.co_reg, a2.tx_tiptra, a2.co_emptra, c1.tx_nom AS tx_nomEmp";
                                            strFecCar+="      , a2.co_ctatra, round(a2.nd_valtra,2) AS nd_valtraBan, a2.st_regrep, a2.co_reg, b3.fe_car, b3.co_ctabanegr";
                                            strFecCar+="      , b3.co_ctabaning, round(b3.nd_valtra,2) AS nd_valtra, b3.tx_valtrapal, b3.st_mosparresp, b3.tx_mottra";
                                            strFecCar+="      , b3.tx_ejeasi, b3.st_fircon, b3.tx_firaut1, b3.tx_carfiraut1, b3.tx_firaut2, b3.tx_carfiraut2";
                                            strFecCar+=" FROM tbm_cabOtrMovBan AS a1 INNER JOIN";
                                            strFecCar+=" (  (  tbm_detOtrMovBan AS a2  INNER JOIN tbm_emp AS c1  ON a2.co_empTra=c1.co_emp)";
                                            strFecCar+="     LEFT OUTER JOIN tbm_docgenotrmovban AS b2";
                                            strFecCar+="     ON a2.co_emp=b2.co_emp AND a2.co_loc=b2.co_loc AND a2.co_tipDoc=b2.co_tipDoc AND a2.co_doc=b2.co_doc AND a2.co_reg=b2.co_reg";
                                            strFecCar+="     LEFT OUTER JOIN tbm_cartrabandia AS b3";
                                            strFecCar+="     ON b2.co_empRelCabDia=b3.co_emp AND b2.co_locRelCabDia=b3.co_loc AND b2.co_tipDocRelCabDia=b3.co_tipDoc AND b2.co_docRelCabDia=b3.co_dia";
                                            strFecCar+=" )  ON a1.co_emp=a2.co_emp AND a1.co_loc=a2.co_loc AND a1.co_tipDoc=a2.co_tipDoc AND a1.co_doc=a2.co_doc";
                                            strFecCar+=" WHERE a2.tx_tipTra='E'";
                                            strFecCar+=" AND a1.co_emp=" + objUti.getIntValueAt(arlDatCon, intIndiceCon, INT_ARL_CON_COD_EMP);
                                            strFecCar+=" AND a1.co_loc=" + objUti.getIntValueAt(arlDatCon, intIndiceCon, INT_ARL_CON_COD_LOC);
                                            strFecCar+=" AND a1.co_tipDoc=" + objUti.getIntValueAt(arlDatCon, intIndiceCon, INT_ARL_CON_COD_TIPDOC);
                                            strFecCar+=" AND a1.co_doc=" + objUti.getIntValueAt(arlDatCon, intIndiceCon, INT_ARL_CON_COD_DOC);
                                            strFecCar+=" AND a2.co_reg=" + objTblMod.getValueAt(k, INT_TBL_DAT_COD_SEC) + "";
                                            strFecCar+=" ORDER BY a2.co_emp, a2.co_loc, a2.co_tipdoc, a2.co_doc, a2.co_reg";
                                    ...
                                    case 428: 
                                        //<editor-fold defaultstate="collapsed" desc="/* Carta de transferencias locales (Pichincha-Miami)... */"> 
                                        if(strTipTrs.equals("Egreso bancario")){
                                            stmRpt=conRpt.createStatement();
                                            strSQL ="";
                                            strSQL+=" SELECT a2.co_emp, a2.co_loc, a2.co_tipdoc, a2.co_doc, a2.co_reg, a2.tx_tiptra, a2.co_emptra, c1.tx_nom AS tx_nomEmp";
                                            strSQL+="      , a2.co_ctatra, round(a2.nd_valtra,2) AS nd_valtraBan, a2.st_regrep, a3.co_cta, a3.tx_codCta, a3.tx_desLar AS tx_nomCta";
                                            strSQL+="      , a4.tx_nom AS tx_nomEmp, a2.co_reg, b3.fe_car, b3.co_ctabanegr";
                                            strSQL+="      , b3.co_ctabaning, round(b3.nd_valtra,2) AS nd_valtra, b3.tx_valtrapal";
                                            strSQL+="      , CASE WHEN b3.st_mosparresp IS NULL THEN '' ELSE b3.st_mosparresp END AS st_mosparresp, b3.tx_mottra";
                                            strSQL+="      , b3.tx_ejeasi, b3.st_fircon, b3.tx_firaut1, b3.tx_carfiraut1, b3.tx_firaut2, b3.tx_carfiraut2";
                                            strSQL+="      , b4.co_ban AS co_banIng, b4.tx_numctaban AS tx_numctabanIng, b5.tx_desCor AS tx_desCorBcoIng, b5.tx_desLar AS tx_desLarBcoIng";
                                            strSQL+="      , b6.co_ban AS co_banEgr, b6.tx_numctaban AS tx_numctabanEgr, b7.tx_desCor AS tx_desCorBcoEgr, b7.tx_desLar AS tx_desLarBcoEgr";
                                            strSQL+="      , b4.co_ctaprvloc, c2.tx_nom AS tx_nomPrvLoc, CASE WHEN b6.tx_tipctaban='C' THEN 'cta. cte.' ELSE 'Cuenta de Ahorros' END AS tx_tipctabanEgr";
                                            strSQL+="      , CASE WHEN b4.tx_tipctaban='C' THEN 'cta.cte.' ELSE 'cta.aho.' END AS tx_tipctabanIng";
                                            strSQL+="      , a4.tx_ruc AS tx_rucEmp, a4.tx_dir AS tx_dirEmp, b4.tx_rucban as ruc_bco_egreso";
                                            ...
                                            strSQL+="      , a1.tx_obs2, b4.tx_codSwi AS tx_codSwiIng, b4.tx_codIba AS tx_codIbaIng, b4.tx_codAba AS tx_codAbaIng";
                                            strSQL+="      , b6.st_fircon, b6.tx_firaut1, b6.tx_carfiraut1, b6.tx_firaut2, b6.tx_carfiraut2";
                                            strSQL+=" FROM tbm_cabOtrMovBan AS a1 INNER JOIN";
                                            strSQL+=" (   (  tbm_detOtrMovBan AS a2  INNER JOIN tbm_emp AS c1  ON a2.co_empTra=c1.co_emp)";
                                            strSQL+="        LEFT OUTER JOIN tbm_docgenotrmovban AS b2";
                                            strSQL+="        ON a2.co_emp=b2.co_emp AND a2.co_loc=b2.co_loc AND a2.co_tipDoc=b2.co_tipDoc AND a2.co_doc=b2.co_doc AND a2.co_reg=b2.co_reg";
                                            strSQL+="        LEFT OUTER JOIN (	( tbm_cartrabandia AS b3";
                                            strSQL+="                             INNER JOIN (tbm_ctaban AS b4 LEFT OUTER JOIN tbm_cli AS c2 ON b4.co_emp=c2.co_emp AND b4.co_ctaprvloc=c2.co_cli)";
                                            strSQL+="                             ON b3.co_emp=b4.co_emp AND b3.co_ctaBanIng=b4.co_ctaban";
                                            ...
                                            strSQL+=" AND a1.co_doc=" + objUti.getIntValueAt(arlDatCon, intIndiceCon, INT_ARL_CON_COD_DOC);
                                            strSQL+=" AND a2.co_reg=" + objTblMod.getValueAt(k, INT_TBL_DAT_COD_SEC) + "";
                                            strSQL+=" AND (b7.tx_desLar LIKE lower('%ichin%iami%') OR b7.tx_desLar LIKE upper('%PICHIN%IAMI%')  )";
                                            strSQL+=" ORDER BY a2.co_emp, a2.co_loc, a2.co_tipdoc, a2.co_doc, a2.co_reg";
                                            rstRpt=stmRpt.executeQuery(strSQL);
                                            if(rstRpt.next())
                                                blnRpt=true;
                                            ...
                                            strFecCar+=" WHERE f2.tx_tipTra='E'";
                                    ...
                                    case 430:
                                        //<editor-fold defaultstate="collapsed" desc="/* Carta de transferencias locales (Produbank Panama)... */"> 
                                        if(strTipTrs.equals("Egreso bancario")){
                                            stmRpt=conRpt.createStatement();
                                            strSQL ="";
                                            strSQL+=" SELECT a2.co_emp, a2.co_loc, a2.co_tipdoc, a2.co_doc, a2.co_reg, a2.tx_tiptra, a2.co_emptra, c1.tx_nom AS tx_nomEmp";
                                            ...
                                            strSQL+="      , b4.co_ban AS co_banIng, b4.tx_numctaban AS tx_numctabanIng, b5.tx_desCor AS tx_desCorBcoIng, b5.tx_desLar AS tx_desLarBcoIng";
                                            strSQL+="      , b6.co_ban AS co_banEgr, b6.tx_numctaban AS tx_numctabanEgr, b7.tx_desCor AS tx_desCorBcoEgr, b7.tx_desLar AS tx_desLarBcoEgr";
                                            strSQL+="      , b4.co_ctaprvloc, c2.tx_nom AS tx_nomPrvLoc, c2.tx_dir AS tx_dirPrvLoc, c2.tx_tel AS tx_telPrvLoc, c2.tx_ide AS tx_rucPrvLoc";
                                            strSQL+="      , CASE WHEN b6.tx_tipctaban='C' THEN 'cta. cte.' ELSE 'Cuenta de Ahorros' END AS tx_tipctabanEgr";
                                            strSQL+="      , CASE WHEN b4.tx_tipctaban='C' THEN 'cta.cte.' ELSE 'cta.aho.' END AS tx_tipctabanIng";
                                            strSQL+="      , a4.tx_ruc AS tx_rucEmp, a4.tx_dir AS tx_dirEmp, b4.tx_rucban as ruc_bco_egreso";
                                            strSQL+="      , CAST('Guayaquil' AS CHARACTER VARYING) as ciuRem";
                                            strSQL+="      , CAST('Guayas' AS CHARACTER VARYING) as prvRem";
                                            ...
                                            strSQL+="      , a1.tx_obs2, b4.tx_codSwi AS tx_codSwiIng, b4.tx_codIba AS tx_codIbaIng, b4.tx_codAba AS tx_codAbaIng";
                                            strSQL+="      , b6.st_fircon, b6.tx_firaut1, b6.tx_carfiraut1, b6.tx_firaut2, b6.tx_carfiraut2";
                                            strSQL+="      , a7.tx_desLar AS tx_nomCiuEgr, a8.tx_desLar AS tx_nomPaiEgr";
                                            strSQL+=" FROM tbm_cabOtrMovBan AS a1 INNER JOIN";
                                            strSQL+=" (  (  tbm_detOtrMovBan AS a2 INNER JOIN tbm_emp AS c1  ON a2.co_empTra=c1.co_emp)";
                                            strSQL+="    LEFT OUTER JOIN tbm_docgenotrmovban AS b2 ON a2.co_emp=b2.co_emp AND a2.co_loc=b2.co_loc AND a2.co_tipDoc=b2.co_tipDoc";
                                            strSQL+="    AND a2.co_doc=b2.co_doc AND a2.co_reg=b2.co_reg";
                                            strSQL+="    LEFT OUTER JOIN ( ( tbm_cartrabandia AS b3";
                                            strSQL+="                        INNER JOIN (tbm_ctaban AS b4 LEFT OUTER JOIN tbm_cli AS c2 ON b4.co_emp=c2.co_emp AND b4.co_ctaprvloc=c2.co_cli)";
                                            strSQL+="                        ON b3.co_emp=b4.co_emp AND b3.co_ctaBanIng=b4.co_ctaban";
                                            strSQL+="                        LEFT OUTER JOIN tbm_var AS b5 ON b4.co_ban=b5.co_reg";
                                            ...
                                            strSQL+="                       INNER JOIN tbm_ctaban AS b6 ON b3.co_emp=b6.co_emp AND b3.co_ctaBanEgr=b6.co_ctaban";
                                            strSQL+="                       LEFT OUTER JOIN tbm_var AS b7 ON (b6.co_ban=b7.co_reg)";
                                            strSQL+="     		    LEFT OUTER JOIN tbm_ciu AS a7 ON (b6.co_ciuCtaBan=a7.co_ciu)";
                                            strSQL+="  			    LEFT OUTER JOIN tbm_pai AS a8 ON (a7.co_pai=a8.co_pai)";
                                            strSQL+="     ) ON b2.co_empRelCabDia=b3.co_emp AND b2.co_locRelCabDia=b3.co_loc";
                                            strSQL+="       AND b2.co_tipDocRelCabDia=b3.co_tipDoc AND b2.co_docRelCabDia=b3.co_dia";
                                            strSQL+=" ) ON a1.co_emp=a2.co_emp AND a1.co_loc=a2.co_loc AND a1.co_tipDoc=a2.co_tipDoc AND a1.co_doc=a2.co_doc";
                                            strSQL+=" INNER JOIN tbm_plaCta AS a3 ON a2.co_empTra=a3.co_emp AND a2.co_ctaTra=a3.co_cta";
                                            strSQL+=" INNER JOIN tbm_emp AS a4 ON a2.co_empTra=a4.co_emp";
                                            strSQL+=" WHERE a2.tx_tipTra='E'";
                                            ...
                                            strSQL+=" AND a1.co_doc=" + objUti.getIntValueAt(arlDatCon, intIndiceCon, INT_ARL_CON_COD_DOC);
                                            strSQL+=" AND a2.co_reg=" + objTblMod.getValueAt(k, INT_TBL_DAT_COD_SEC) + "";
                                            strSQL+=" AND (b7.tx_desLar LIKE lower('%rodubank%') OR b7.tx_desLar LIKE upper('%RODUBANK%')  )";
                                            strSQL+=" ORDER BY a2.co_emp, a2.co_loc, a2.co_tipdoc, a2.co_doc, a2.co_reg";
                                            rstRpt=stmRpt.executeQuery(strSQL);
                                            if(rstRpt.next())
                                                blnRpt=true;
