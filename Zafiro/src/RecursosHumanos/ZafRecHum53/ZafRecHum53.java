...
package RecursosHumanos.ZafRecHum53;
 ...
 * Guayaquil 05/09/2012
 */
public class ZafRecHum53 extends javax.swing.JInternalFrame
{
    ...
    private static final int INT_TBL_DAT_CHK_FAL=11;
    private static final int INT_TBL_DAT_CHK_JUS_PER=12;
    private static final int INT_TBL_DAT_CHK_JUS_FAL=13;
    private static final int INT_TBL_DAT_HOR_ENT_JUS=14;
    ...
    private static final int INT_TBL_DAT_ST_JUS_FAL=21;    
    ...
    private ZafTblCelEdiChk zafTblCelEdiChkLab;                             //Editor de Check Box para campo Tiene Falta
    private ZafTblCelRenChk zafTblCelRenChkLab;                             //Renderer de Check Box para campo Tiene Falta
    private ZafTblCelEdiChk zafTblCelEdiChkLabPer;                          //Editor de Check Box para campo Permiso
    private ZafTblCelRenChk zafTblCelRenChkLabPer;                          //Renderer de Check Box para campo Permiso
    private ZafTblCelEdiChk zafTblCelEdiChkLabFal;                          //Editor de Check Box para campo Justificación Falta
    private ZafTblCelRenChk zafTblCelRenChkLabFal;                          //Renderer de Check Box para campo Justificación Falta
...
//    private ZafMouMotAdaMovReg objMouMotAdaMovReg;                        //ToolTipText en TableHeader.
    ...
    /** Crea una nueva instancia de la clase ZafRecHum53F. */
    public ZafRecHum53(ZafParSis obj)
    {
        try
        {
            initComponents();
    ...
    private void initComponents() {
        ...
        jChkEmpTieFal = new javax.swing.JCheckBox();
        jChkEmpNoTieFal = new javax.swing.JCheckBox();
        ...
        jChkEmpTieFal.setSelected(true);
        jChkEmpTieFal.setText("Mostrar los empleados que tienen faltas");
        jChkEmpTieFal.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jChkEmpTieFalActionPerformed(evt);
            }
        });
        panFil.add(jChkEmpTieFal);
        jChkEmpTieFal.setBounds(24, 196, 400, 23);

        jChkEmpNoTieFal.setText("Mostrar los empleados que no tienen faltas");
        jChkEmpNoTieFal.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jChkEmpNoTieFalActionPerformed(evt);
            }
        });
        panFil.add(jChkEmpNoTieFal);
        jChkEmpNoTieFal.setBounds(24, 216, 400, 23);
        ...
        panBot.setPreferredSize(new java.awt.Dimension(199, 35));
            ...
            if(con!=null){
                ...
                for(int i=0; i<tblDat.getRowCount();i++){
                    ...
                    if(tblDat.getValueAt(i, INT_TBL_DAT_LIN).toString().compareTo("M")==0){
                        ...
                        intCoTraSel=Integer.parseInt(objTblMod.getValueAt(i, INT_TBL_DAT_COD_TRA).toString());
                        if( (intCoTraSel==Integer.parseInt(objTblMod.getValueAt(i, INT_TBL_DAT_COD_TRA).toString())) && strCoMot.compareTo("4")==0){
                            intCon++;
                            bolDevIESS = true;
                        }
                        
                        String strJusFal="null";
                        if(objTblMod.getValueAt(i, INT_TBL_DAT_CHK_JUS_PER).equals(true) && objTblMod.getValueAt(i, INT_TBL_DAT_CHK_JUS_FAL).equals(false)){
                            strJusFal=objUti.codificar("P");
                        }else if(objTblMod.getValueAt(i, INT_TBL_DAT_CHK_JUS_FAL).equals(true) && objTblMod.getValueAt(i, INT_TBL_DAT_CHK_JUS_PER).equals(false)){
                            strJusFal=objUti.codificar("S");
                        }
                        //David
                        if(!objTblMod.getValueAt(i, INT_TBL_DAT_ST_JUS_FAL).toString().equals("0")){
                            mostrarMsgInf("No puede Desjustificar el registro.");
                            blnRes = false ;
                            break;
                        }
                        if(strJusFal.length() == 3 ){
                             ...
                             if (valorDevolver > 0){
                                 String txObs = "Zafiro: Justificación Faltas (" + tblDat.getValueAt(i, INT_TBL_DAT_FECHA).toString() + ")" ;
                        ...
                        strSql+=" and fe_dia="+objUti.codificar(tblDat.getValueAt(i, INT_TBL_DAT_FECHA));
                        rstLoc=stmLoc.executeQuery(strSql);
                ...
                if(bolDevIESS && txtCodTra.getText().length() <= 0){
                    blnRes = false;
                    mostrarMsgInf("Para justificacion de faltas por motivo del IESS, consulte el empleado con el filtro.");
                    
                }
                if (txtCodTra.getText().length() > 0)
                if(Integer.parseInt(txtCodTra.getText())==intCoTraSel){
                        if(intCon>3){
                            strSql="";
                            strSql+="SELECT a.* , b.co_emp ,round (((("+(intCon-3)+")*(nd_valrub/30))*0.75),2) as nd_valRubDescIESS FROM tbh_suetra a "+"\n";
...
//                            strSql+="SELECT * , round (((("+(intCon-3)+")*(nd_valrub/30))*0.75),2) as nd_valRubDescIESS FROM tbh_suetra a "+"\n";
                            strSql+="WHERE a.co_tra="+intCoTraSel+"\n";
                            ...
                            if(rstLocAux.next()){
                                ...
                                String strSQL="SELECT case when (Max(co_egr)+1) is null then 1 else Max(co_egr)+1 end as co_egr  FROM tbm_cabingegrprgtra WHERE " +
                                        " co_emp="+rstLocAux.getString("co_emp");
                                 rstLoc = stmLoc.executeQuery(strSQL);
                                 if(rstLoc.next())
                                ...
                                strSql+="VALUES ( "+"\n";
                                strSql+=rstLocAux.getString("co_emp") +" , \n";
                                strSql+=intCoEgr +" , \n";
                                strSql+=intCoTraSel +" , \n";
                                strSql+="29"+" , \n";
                                strSql+="current_date" +" , \n";
                                ...
                                strSql+=objUti.codificar(strFePriCuo) +" , \n";
                                strSql+=rstLocAux.getString("nd_valRubDescIESS") +" , \n";
    ...
    private void jChkEmpNoTieFalActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jChkEmpNoTieFalActionPerformed
        // TODO add your handling code here:
        jChkEmpTieFal.setSelected(Boolean.FALSE);
    }//GEN-LAST:event_jChkEmpNoTieFalActionPerformed
            ...
            if(txtHorSalJus.getText().compareTo("")!=0){
                ...
                if((intHH>=0 && intHH<=24)){
                    if(!(intMM>=0 && intMM<=59)){
                        ...
                        JOptionPane.showMessageDialog(ZafRecHum53.this,strMen,strTit,JOptionPane.INFORMATION_MESSAGE);
                    }else{
        ...
        if (mostrarMsgCon("¿Está seguro que desea realizar esta operación?")==0){
            ...
            for(int intFil=0; intFil < tblDat.getRowCount(); intFil++){
                ...
                if(objTblMod.getValueAt(intFil, INT_TBL_DAT_CHKSEL).equals(true)){
                    ...
                    objTblMod.setValueAt(Boolean.TRUE, intFil, INT_TBL_DAT_CHK_FAL);
                    ...
                    objTblMod.setValueAt(Boolean.TRUE, intFil, INT_TBL_DAT_CHK_JUS_FAL);
                }
            }
        }
    ...
    private void jChkEmpTieFalActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jChkEmpTieFalActionPerformed
        // TODO add your handling code here:
        jChkEmpNoTieFal.setSelected(Boolean.FALSE);
    }//GEN-LAST:event_jChkEmpTieFalActionPerformed
    ...
    private javax.swing.JCheckBox jChkEmpNoTieFal;
    private javax.swing.JCheckBox jChkEmpTieFal;
...
//            }
    ...
    private boolean configuraTbl(){
        ...
        vecCab.add(INT_TBL_DAT_CHK_FAL,"¿Falta?");
        vecCab.add(INT_TBL_DAT_CHK_JUS_PER,"Permiso");
        vecCab.add(INT_TBL_DAT_CHK_JUS_FAL,"Justificar");
        ...
        tcmAux.getColumn(INT_TBL_DAT_CHK_FAL).setPreferredWidth(80);
        tcmAux.getColumn(INT_TBL_DAT_CHK_JUS_PER).setPreferredWidth(80);
        tcmAux.getColumn(INT_TBL_DAT_CHK_JUS_FAL).setPreferredWidth(80);
        ...
        objTblMod.addSystemHiddenColumn(INT_TBL_DAT_ST_JUS_FAL, tblDat);
        ...
        vecAux.add("" + INT_TBL_DAT_CHK_JUS_PER);
        vecAux.add("" + INT_TBL_DAT_CHK_JUS_FAL);
        ...
        zafTblCelRenChkLab = new ZafTblCelRenChk();
        tcmAux.getColumn(INT_TBL_DAT_CHK_FAL).setCellRenderer(zafTblCelRenChkLab);
        zafTblCelEdiChkLab = new ZafTblCelEdiChk();
        tcmAux.getColumn(INT_TBL_DAT_CHK_FAL).setCellEditor(zafTblCelEdiChkLab);
        
        zafTblCelRenChkLabPer = new ZafTblCelRenChk();
        tcmAux.getColumn(INT_TBL_DAT_CHK_JUS_PER).setCellRenderer(zafTblCelRenChkLabPer);
        zafTblCelEdiChkLabPer = new ZafTblCelEdiChk();
        tcmAux.getColumn(INT_TBL_DAT_CHK_JUS_PER).setCellEditor(zafTblCelEdiChkLabPer);

         zafTblCelEdiChkLabPer.addTableEditorListener(new Librerias.ZafTblUti.ZafTblEvt.ZafTableAdapter() {
            ...
            public void afterEdit(Librerias.ZafTblUti.ZafTblEvt.ZafTableEvent evt) {
                ...
                objTblMod.setValueAt(Boolean.TRUE, intFil, INT_TBL_DAT_CHK_JUS_PER);
                objTblMod.setValueAt(Boolean.FALSE, intFil, INT_TBL_DAT_CHK_JUS_FAL);
            }
         });
        
        zafTblCelRenChkLabFal = new ZafTblCelRenChk();
        tcmAux.getColumn(INT_TBL_DAT_CHK_JUS_FAL).setCellRenderer(zafTblCelRenChkLabFal);
        zafTblCelEdiChkLabFal = new ZafTblCelEdiChk();
        tcmAux.getColumn(INT_TBL_DAT_CHK_JUS_FAL).setCellEditor(zafTblCelEdiChkLabFal);

         zafTblCelEdiChkLabFal.addTableEditorListener(new Librerias.ZafTblUti.ZafTblEvt.ZafTableAdapter() {
            ...
            public void afterEdit(Librerias.ZafTblUti.ZafTblEvt.ZafTableEvent evt) {
                int intFil = tblDat.getSelectedRow();
                objTblMod.setValueAt(Boolean.FALSE, intFil, INT_TBL_DAT_CHK_JUS_PER);
                objTblMod.setValueAt(Boolean.TRUE, intFil, INT_TBL_DAT_CHK_JUS_FAL);
            }
         });
       ...
       Librerias.ZafTblUti.ZafTblCelRenBut.ZafTblCelRenBut zafTblDocCelRenBut = new ZafTblCelRenBut();
        ...
        ZafTableColBut_uni zafTableColBut_uni = new ZafTableColBut_uni(tblDat, INT_TBL_DAT_BUT_OBS, "Observación") {
            public void butCLick() {
                ...
                ZafMae07_01 zafMae07_01 = new ZafMae07_01(JOptionPane.getFrameForComponent(ZafRecHum53.this), true, strObs);
                zafMae07_01.show();
...
//        tcmAux.getColumn(INT_TBL_DAT_CHK_JUS_FAL).setCellRenderer(objTblCelRenChkLab);
//        objTblCelEdiChk=new ZafTblCelEdiChk();
//        tcmAux.getColumn(INT_TBL_DAT_CHK_JUS_FAL).setCellEditor(objTblCelEdiChk);
...
////                if(objTblMod.getValueAt(intFilSel, INT_TBL_DAT_CHK_JUS_FAL).equals(true)){
...
//                objTblHeaGrp.addColumnGroup(objTblHeaColGrpEmp);
                ...
                objTblHeaColGrpEmp.add(tblDat.getColumnModel().getColumn(INT_TBL_DAT_CHK_JUS_PER+i*INT_TBL_DAT_NUM_TOT_CDI));
...
private boolean cargarDat(){
  ...
  try{
      ...
      if(conn!=null){   
          ...
          //David
//          String strMarInc="";
//          if(optFil.isSelected()){//jChkTodEmp
//              if(jChkEmpNoTieFal.isSelected() && jChkEmpTieFal.isSelected()){
...
//                  strMarInc+="  ((f.ho_ent is null and f.ho_sal is null) or";
//                strMarInc+=" not (f.ho_ent is null and f.ho_sal is null)) ";
//            }
//            if(jChkEmpNoTieFal.isSelected() && !jChkEmpTieFal.isSelected()){
//                strMarInc+="  not (f.ho_ent is null and f.ho_sal is null) ";
//            }
//          
//            if(!jChkEmpNoTieFal.isSelected() && jChkEmpTieFal.isSelected()){
//                strMarInc+="  (f.ho_ent is null and f.ho_sal is null) ";
//            }
//          }
          //if(objParSis.getCodigoEmpresa()==objParSis.getCodigoEmpresaGrupo()){
          ...
          if(objParSis.getCodigoUsuario()!=1){
              strDepAut="inner join tbr_depprgusr h on(h.co_dep=d.co_dep and h.co_dep in (select co_dep from tbr_depprgusr where co_usr = "+objParSis.getCodigoUsuario()+" "+
          ...
          strSql=" select * from (";
          strSql+="select coalesce(st_jusfal,'0') as st_jus, d.co_emp,tx_empresa,d.co_tra,d.co_ofi,d.co_dep,d.co_hor,empleado,e.fe_dia, e.ho_ent as horaEntradaEstablecida,e.ho_sal as horaSalidaEstablecida," + "\n";
...
//          strSql+=" (case st_jusFal when 'S' then true else false end) as blnJusFal";
          ...
          //if(jChkEmpTieFal.isSelected()){//David 
              strSql+=" inner join tbm_cabconasitra f on (f.co_tra=d.co_tra and f.fe_dia=e.fe_dia)" + "\n";
//          }
//          if(jChkEmpNoTieFal.isSelected()){
//              strSql+=" INNER JOIN tbm_cabconasitra f on (f.co_tra=d.co_tra and f.fe_dia=e.fe_dia)" + "\n";
//          }
...
//          strSql+=" " + strMarInc+ "\n";
          ...
          String strSqlFalta = "where blnTieFal=" + jChkEmpTieFal.isSelected() ;
          if (!jChkEmpTieFal.isSelected() && !jChkEmpNoTieFal.isSelected() )
              strSqlFalta = "";
          ...
          strSql+=" order by e.fe_dia , empleado" + " ) x " + strSqlFalta;
          //}
          ...
          //if(rstLoc.next()){
            ...
            while(rstLoc.next()){
                ...
                vecReg.add(INT_TBL_DAT_CHK_FAL,rstLoc.getBoolean("blnTieFal"));
                
                String strStJusFal = rstLoc.getString("st_jusfal");
                if(strStJusFal==null){
                    vecReg.add(INT_TBL_DAT_CHK_JUS_PER,Boolean.FALSE);
                    vecReg.add(INT_TBL_DAT_CHK_JUS_FAL,Boolean.FALSE);
                }else if(strStJusFal.compareTo("S")==0){
                    vecReg.add(INT_TBL_DAT_CHK_JUS_PER,Boolean.FALSE);
                    vecReg.add(INT_TBL_DAT_CHK_JUS_FAL,Boolean.TRUE);
                }else if(strStJusFal.compareTo("P")==0){
                    vecReg.add(INT_TBL_DAT_CHK_JUS_PER,Boolean.TRUE);
                    vecReg.add(INT_TBL_DAT_CHK_JUS_FAL,Boolean.FALSE);
                }
                ...
                vecReg.add(INT_TBL_DAT_COD_MOT,rstLoc.getString("co_motJusFal"));
                vecReg.add(INT_TBL_DAT_BUT_MOT,"");

                vecReg.add(INT_TBL_DAT_OBS,rstLoc.getString("tx_obsJusFal"));
                vecReg.add(INT_TBL_DAT_BUT_OBS,"");
                vecReg.add(INT_TBL_DAT_ST_JUS_FAL,  rstLoc.getString("st_jus"));
        ...
        public void mouseMoved(java.awt.event.MouseEvent evt)
        {
            ...
            switch (intCol)
            {
                ...
                case INT_TBL_DAT_HOR_SAL_MAR:
                    ...
                    break;
                case INT_TBL_DAT_CHK_FAL:
                    strMsg="¿Tiene falta?";
                    break;
                case INT_TBL_DAT_CHK_JUS_PER:
                    strMsg="Permiso:La falta si se descuenta";
                    break;
                case INT_TBL_DAT_CHK_JUS_FAL:
                    strMsg="Justificar: La falta no se descuenta";
                    break;
                case INT_TBL_DAT_HOR_ENT_JUS:
...
//            vcoTra.setConfiguracionColumna(4, javax.swing.JLabel.RIGHT);
    ...
    public double getValorDevolver(Connection conex, String coEmpresa, String coTra, String fecAJustificar, String stJusmarinc ){
        ...
        try {
            String strQurey = "select 1, st_jusmarinc, ne_per, ne_ani, ne_mes from tbm_fecCorRolPag f, tbm_cabconasitra a \n" +
                        ...
                        "	and st_revfal = 'S' and st_jusfal is null";
            CallableStatement cs = conex.prepareCall(strQurey);
            ResultSet rs = cs.executeQuery();
            if(rs.next()){
                if (rs.getInt("ne_per") == 2){
                    valorDevolver = calcularValor(conex, coEmpresa, coTra, fecAJustificar, stJusmarinc);
                }else{
    ...
    public double calcularValor(Connection conex, String coEmpresa, String coTra, String fecAJustificar, String stJusmarinc ) {
        ...
        if(c.get(Calendar.DAY_OF_WEEK) == 7 ){//Sabado
            ...
            valorDevolver = (valorDevolver / 4 );
        }else{
            valorDevolver = getValorRubroEmp(conex, coEmpresa, coTra, "1");
            valorDevolver = (valorDevolver / 30 );
            if(stJusmarinc.equals("'S'")){
                valorDevolver = valorDevolver * 2 ;
            }
        }
        return valorDevolver;
    }
