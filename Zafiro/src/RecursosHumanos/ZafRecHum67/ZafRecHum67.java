...
package RecursosHumanos.ZafRecHum67;
 ...
 * Guayaquil 28/06/2013
 */
public class ZafRecHum67 extends javax.swing.JInternalFrame
{
    ...
    public ZafRecHum67(ZafParSis obj)
    {
        try
        {
            initComponents();
            ...
            if(objParSis.getCodigoUsuario()!=1){
                ...
                if(objPerUsr.isOpcionEnabled(3630)){
                    butCon.setVisible(true);
                }
                if(objPerUsr.isOpcionEnabled(3631)){
                    butGua.setVisible(true);
                }
                if(objPerUsr.isOpcionEnabled(3632)){
            ...
            if(con!=null){
                ...
                for(int i=0; i<tblDat.getRowCount();i++){
                    ...
                    if(tblDat.getValueAt(i, INT_TBL_DAT_LIN).toString().compareTo("M")==0){
                        ...
                        String strNomCol =null;
                        
                        for(int intFil2 = INT_TBL_DAT_NOM_APE_TRA+2; intFil2 < objTblMod.getColumnCount(); intFil2=intFil2+2){
                            System.out.println("at this moment ---> " + objTblMod.getValueAt( i , intFil2).toString());
                            boolean bln = (Boolean)objTblMod.getValueAt( i , intFil2);
                            if(bln){
                                strNomCol = objTblMod.getColumnName(intFil2-1);
                            }
                        }
                        
                        if(strNomCol.compareTo("")==0){
                            strNomCol =null;
                        }
                        
                        strSql="";
                        strSql+="update tbm_traemp";
                        strSql+=" set co_ofi = " + strNomCol ;
        ...
        try{
            ...
            try{
                ...
                if(conIns!=null){
                    ...
                    strSQL="select * from tbm_ofi where st_reg = 'A' order by co_ofi ";
                    System.out.println("sql ne_ani : "+strSQL);
                    rstLoc=stmLoc.executeQuery(strSQL);
                    
                    if(rstLoc.next()){
                        do{
                            vecCab.add(vecCab.size(),rstLoc.getString("co_ofi"));
            ...
            }catch(SQLException ex)
            {
                ...
                try{conIns.close();conIns=null;}catch(Throwable ignore){}
           }
            ...
            final int intColFij = INT_TBL_DAT_NOM_APE_TRA+1;
            for(int intFil=INT_TBL_DAT_NOM_APE_TRA+1; intFil < objTblMod.getColumnCount();intFil++){
                if(intFil%2!=0){
                    objTblMod.addSystemHiddenColumn(intFil, tblDat);
                    tcmAux.getColumn(intFil).setCellRenderer(objTblCelRenLbl);

                    tcmAux.getColumn(intFil).setPreferredWidth(30);
                }else{
                    vecAux.add("" + intFil);
                    tcmAux.getColumn(intFil).setPreferredWidth(70);
                    
                     zafTblCelEdiChkSel = new ZafTblCelEdiChk();
                     tcmAux.getColumn(intFil).setCellEditor(zafTblCelEdiChkSel);
                     zafTblCelEdiChkSel.addTableEditorListener(new Librerias.ZafTblUti.ZafTblEvt.ZafTableAdapter() {
                        ...
                        public void afterEdit(Librerias.ZafTblUti.ZafTblEvt.ZafTableEvent evt) {
                            ...
                            int intCol = tblDat.getSelectedColumn();
                            int intFil = tblDat.getSelectedRow();
                            for(int x=intColFij; x < intMaxCol; x++){

                                if(x!=intCol){
                                    if(x%2==0){
                                        tblDat.setValueAt( new Boolean(false), intFil , x);
                                    }
                                }
                            }
                        }
  ...
  try{
      ...
      if(conn!=null){
          ...
          while(rstLoc.next()){
                ...
                for(int i = INT_TBL_DAT_NOM_APE_TRA+1; i < objTblMod.getColumnCount(); i=i+2){
                    String strNomCol = objTblMod.getColumnName(i);
                    String strCoOfi = rstLoc.getString("co_ofi");
                    if(strCoOfi!=null){
                        if(strNomCol.equals(strCoOfi)){
                        vecReg.add(vecReg.size(), rstLoc.getString("co_ofi"));
                        vecReg.add(vecReg.size(), Boolean.TRUE);
                        }else{
                            vecReg.add(vecReg.size(), "");
                            vecReg.add(vecReg.size(), Boolean.FALSE);
                        }
                    }else{
                        vecReg.add(vecReg.size(), rstLoc.getString("co_ofi"));
                        vecReg.add(vecReg.size(), Boolean.FALSE);
    ...
    public class Oficina{
        ...
        public int getCoOfi(){
            return this.intCoOfi;
        }
        public String getStrTxNom(){
            return this.strTxNom;
        }
        
        public void setCoOfi(int intCoOfi){
            this.intCoOfi = intCoOfi;
        }
        public void setStrTxNom(String strTxNom){
            this.strTxNom = strTxNom;
        }
    }
}