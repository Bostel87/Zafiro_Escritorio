/*
 *    zafVen01_OC.java  co_ben, tx_benchq 
 *
...
public class ZafVen01_OC extends javax.swing.JDialog { // extends javax.swing.JInternalFrame{
    Connection con_remota=null, CONN_GLO=null;      //Coneccion a la base donde se encuentra la cotizacion
    ...
    final int INT_TBL_ITMALT2   = 18 ;
    final int INT_TBL_PRECOSORI = 19;
    final int INT_TBL_ITMSER   = 20;            //Columna que contiene SI el item es de servicio S o N
    final int INT_TBL_NUMREL   = 21;   
    final int INT_TBL_IEBODFIS   = 22;  // estado que dice si ingresa/egresa fisicamente en bodega
...
public ZafVen01_OC(java.awt.Frame parent, boolean modal, Librerias.ZafParSis.ZafParSis obj, int intCodCot2, int intCodPrv2, int intNummmOrc, String codcom, String nomCom , java.sql.Connection con_local, java.sql.Connection con_remoto , int intTipDoc , int intCodDoc , int intNumConCen, int intTipVis ) {
  super(parent, modal);
         ...
         try
          {
           ...
           {
              stmTmp = conTmp.createStatement();
              String sql = "SELECT distinct(co_bod) FROM tbr_bodLoc WHERE co_emp="+objZafParSis.getCodigoEmpresa(); //+" and co_loc="+objZafParSis.getCodigoLocal();
              rst=stmTmp.executeQuery(sql);
              while(rst.next()){
   ...
   public void Configura_ventana_consulta(){
       ...
       private boolean configurarVenConProveedor()
    {
        ...
        try
        {
            ...
            strSQL="SELECT  a.co_cli,tx_nom,tx_dir,tx_tel,tx_ide ,ciu.tx_desLar, a.tx_tipper ,a.co_tipper, a.st_ivacom  FROM tbr_cliloc as a1 " +
  ...
  if(conTipEmp!=null){
   ...
   public int getAccionDoc(){
    ...
    private void initComponents() {
        ...
        butCom.setFont(new java.awt.Font("SansSerif", 0, 11));
        butCom.setText("...");
        ...
        lblNumCot.setFont(new java.awt.Font("SansSerif", 0, 11));
        ...
        lblNumPed.setFont(new java.awt.Font("SansSerif", 0, 11));
        ...
        tblOrdComForPag.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                " Línea", "Dias de crédito", "Fecha de Vencimiento", "% Retención", "Monto de Pago", "Días de gracia", "CodRet", "Cod.Sri.", "tx_apliret"
            }
        ) {
            Class[] types = new Class [] {
                java.lang.Object.class, java.lang.Integer.class, java.lang.String.class, java.lang.String.class, java.lang.Double.class, java.lang.Integer.class, java.lang.Integer.class, java.lang.String.class, java.lang.String.class
            };
            boolean[] canEdit = new boolean [] {
                               ...
                               int i=0;
                                    ...
                                    java.sql.Connection  conn = DriverManager.getConnection(objZafParSis.getStringConexion(),objZafParSis.getUsuarioBaseDatos(),objZafParSis.getClaveBaseDatos());
                                     if (conn!=null){       
                                      ...
                                      strSQL += " where polret.co_emp = "+ objZafParSis.getCodigoEmpresa()+" and co_mot = "+intCodMotDoc+" and co_ageret = " + intCodTipPerEmp +" and co_sujret = "+strCodTipPerCli+" "+
               ...
               //*********************************************************************************************
                   ...
                   /********************************************************************/
                    ...
                    if(dblCos > dblCosOri ){
                        ...
                        String strTit, strMsg;
                        strTit="Mensaje del sistema Zafiro";
                        strMsg="El Costo no puede ser mayor al costo origen .\nDigite el costo correcto.";
                        obj.showMessageDialog(jfrThis,strMsg,strTit,javax.swing.JOptionPane.INFORMATION_MESSAGE);
                        tblDat.repaint();    
                        tblDat.requestFocus();  
                        tblDat.editCellAt(intRowVal, INT_TBL_PRECOS);
    ...
    public void  refrescaDatos(){
        ...
        try{//odbc,usuario,password        
            ...
            " THEN 'S' ELSE 'N' END AS proconf  ";
                  ...
                  sSQL = " SELECT * FROM ( " +
                          " SELECT a.nd_pordesprecom,  a.co_reg, a.co_itm,inv.tx_codalt,a.tx_nomitm,var.tx_descor,a.co_bod,a.nd_can,a.nd_precom,inv.st_ivacom,inv.tx_codalt2, inv.st_ser as servicio ";
                  sSQL +=strAux;
                  sSQL +=strAux2;
      ...
      public boolean insertar()
        {
            ...
            if(!insertarReg(CONN_GLO, con_remota))
                return false;
...
private boolean insertarCab(java.sql.Connection conn, java.sql.Connection connRemoto, int intCodDoc, String strConfInv ){
  boolean blnRes=false;
...
//      }
      ...
      intSecGrp+","+intSecEmp+" ,'I','C','"+strConfInv+"', "+intCodBen+", '"+strNomBen+"' )";  // ", "+(intSecGrp-1)+", "+(intSecEmp-1)+" " +
  ...
  try{
   ...
   for(int i=0; i<tblDat.getRowCount(); i++){  
    if(tblDat.getValueAt(i,INT_TBL_CODITM) != null ){  
     ...
     ",nd_costot, nd_costotgrp, nd_tot , st_regrep, nd_cancon, st_meringegrfisbod )" +
     " VALUES("+objZafParSis.getCodigoEmpresa()+", "+objZafParSis.getCodigoLocal()+", "+txtCodTipDoc.getText()+"," +
     ...
     tblDat.getValueAt(i,INT_TBL_NUMREL).toString()+",'A',"+objZafParSis.getCodigoLocal()+","+
     txtCodTipDoc.getText()+","+intCodDoc+","+(i+1)+","+objZafParSis.getCodigoEmpresa()+")";
     txtsql.append(strSql + "; \n");
  }}
  blnRes=true;
  }catch(Exception evt){ blnRes=false; objUti.mostrarMsgErr_F1(this, evt); }
...
private boolean insertarPag(java.sql.Connection conn, int intCodDoc){
 ...
 try{
    ...
    for(int i=0; i<tblOrdComForPag.getRowCount();i++){                       
       ...
       (strApliRet.equals("S")?txtSub.getText():"0")+" )";   
       txtsql.append(strSql + "; \n");
...
private boolean insertarReg(java.sql.Connection conn, java.sql.Connection connRemota ){
  boolean blnRes=false;
  java.sql.Statement stmLoc;
...
//        }}}
      ...
      //   if(strMerIngEgr.equals("N")) strConfInv="F";
        ...
        if(insertarCab(conn, connRemota, intCodDoc, strConfInv )){    
         if(insertarDet(intCodDoc, strMerIngEgr )){    
          if(insertarPag(conn, intCodDoc )){   
           if(objDiario.insertarDiario_OC(conn, objZafParSis.getCodigoEmpresa(), objZafParSis.getCodigoLocal(), Integer.parseInt(txtCodTipDoc.getText()), String.valueOf(intCodDoc), String.valueOf(intCodDoc), objUti.parseDate(txtFecDoc.getText(),"dd/MM/yyyy"), 45  ) ){   
            if(actualizaStock(conn, connRemota, strMerIngEgr )){
               txtsql.setText(""); 
               blnRes=true;
            }else blnRes=false;   
           }else blnRes=false;    
...
//      
//        public boolean insertarReg_resplado(){
//            
...
//                          
//                          try{
//                                java.sql.PreparedStatement pstInsOrdCom;
//                                java.sql.Statement stmLoc;
//                                java.sql.ResultSet rstLoc;
//                              try{        
//                                    
...
//                                        String strFecha = "#" + Fecha[2] + "/"+Fecha[1] + "/" + Fecha[0] + "#";
//                                        
//                                        txtDoc.setText(""+objUltDocPrint.getCodigoDocumento(CONN_GLO, objTipDoc.getco_tipdoc()));
//                                          if (Integer.parseInt( txtOrdCom.getText())>0){
//                                              if ( objUltDocPrint.getExisteDoc(  Integer.parseInt( txtOrdCom.getText()) ,                                           
...
//                                       ///*******************************************************************///
//                                           
//                                       String sql2= "SELECT st_meringegrfisbod, tx_natdoc FROM tbm_cabtipdoc WHERE  co_emp="+objZafParSis.getCodigoEmpresa()+" "+
//                                        " and co_loc="+objZafParSis.getCodigoLocal()+" and co_tipDoc="+txtCodTipDoc.getText();
//                                        rstLoc = stmLoc.executeQuery(sql2);
//                                        if(rstLoc.next()){
...
//                                               " fe_doc, "                 + //Fecha de la orden de compra
...
//                                               " ne_numDoc, ne_numCot, tx_numPed, " +//<==Campos que aparecen en la parte superior del 1er Tab
//                                               " tx_obs1, tx_obs2, nd_sub , nd_tot, nd_valiva, nd_porIva, " +//<==Campos que aparecen en la parte inferior del 1er Tab
//                                               " co_forPag, " +
//                                               " fe_ing, co_usrIng,fe_ultMod, co_usrMod , tx_desforpag ,st_reg" + //Campos de Auditoria Fecha de Ingreso Y Usuario q ingreso.
//                                               " ,co_forret,tx_vehret,tx_choret,ne_secgrp,ne_secemp , st_regrep , st_tipdev, st_coninv  " +
...
//                                                        intNumDoc + ",'" +
//                                                        strFecha + "'," +
//                                                        Integer.parseInt(txtPrvCod.getText()) + "," + 
//                                                        Integer.parseInt(txtComCod.getText()) + ",'" +
//                                                        txtAte.getText() + "','" +
//                                                        txtPrvNom.getText() + "','" +
//                                                        txtPrvDir.getText() + "','" +
//                                                       
//                                                        txtcli_identificacion.getText()+ "','"+ 
//                                                        txtcli_telefono.getText()+ "','"+ 
//                                                        txtcli_ciudad.getText()+ "','"+ 
//                                                        
//                                                        txtComNom.getText() + "'," +
//                                                        txtOrdCom.getText() + "," +
//                                                        ((txtCot.getText().equals(""))?"0":txtCot.getText()) + "," +
//                                                        objUti.codificar(txtFacPrv.getText()) + ",'" +
//                                                        txaObs1.getText() + "','" + 
//                                                        txaObs2.getText() + "'," + 
//                                                        objUti.redondeo(Double.parseDouble(txtSub.getText()),6) + "," +
//                                                        objUti.redondeo(Double.parseDouble(txtTot.getText()),6) + "," +
//                                                        txtIva.getText() + "," +
//                                                        objUti.redondeo(dblPorIva , 2) + "," +
//                                                        Integer.parseInt(""+vecForPag.get(cboForPag.getSelectedIndex())) + "," + 
//                                                        "'"+ strFecSis+ "'," +
//                                                        objZafParSis.getCodigoUsuario() + "," +
//                                                        "'"+ strFecSis+ "'," +
//                                                        objZafParSis.getCodigoUsuario() + ",'" +
//                                                        cboForPag.getSelectedItem()  + "','R'" +
//                                                        ","+ txtCod.getText() +",'"+ txtVehRet.getText()+"','"+ txtChoRet.getText()+"',"+intSecGrp+","+intSecEmp+" , 'I','C','"+strconinv+"' " +
...
//
//                                           
//                                                sSQL ="INSERT INTO  tbr_cabMovInv(co_emp,co_loc,co_tipdoc,co_doc,st_reg, co_emprel,  co_locrel,co_tipdocrel,co_docrel)"+
//                                               " values("+objZafParSis.getCodigoEmpresa()+","+objZafParSis.getCodigoLocal()+","+
//                                                intTipDocFac+","+intCodDocFac+",'A',"+objZafParSis.getCodigoEmpresa()+","+objZafParSis.getCodigoLocal()+","+
//                                                txtCodTipDoc.getText()+","+intNumDoc+")";
//                                                txtsql.append(sSQL + "; \n");
//                                            
//                                                
...
//                                         
//                                           int intTipCli=3;
//                                               intTipCli= objUltDocPrint.ValidarCodigoCliente(txtPrvCod.getText(),CONN_GLO);
//                                        
//                                             
//                                        /*********************************************\    
//                                         * Armando el Insert para los datos          *   
...
//
//                                             
//    sql =   " INSERT INTO  tbm_detMovInv ";
//    sql += "(co_emp, co_loc, co_tipdoc , co_doc, co_reg, ";  //CAMPOS PrimayKey
//    sql +="tx_unimed, co_itm, co_itmact, tx_codAlt,tx_codAlt2, tx_nomItm, co_bod,  "; //<==Campos que aparecen en la parte superior del 1er Tab
//    sql +=   "nd_can, nd_cosUni, nd_cosUniGrp , nd_preuni, nd_porDes, st_ivaCom ,st_reg"; //<==Campos que aparecen en la parte inferior del 1er Tab
//    sql +=   ",nd_costot, nd_costotgrp, nd_tot , st_regrep, nd_cancon, st_meringegrfisbod )" ;
//    sql +=   "VALUES (";
//     sql +=   objZafParSis.getCodigoEmpresa() + ",";
//     sql +=   objZafParSis.getCodigoLocal() + ",";
//     sql +=   Integer.parseInt(txtCodTipDoc.getText()) + ",";
//     sql +=      intNumDoc + ","; 
//     sql +=      (i+1) + ",'" + tblDat.getValueAt(i, INT_TBL_UNIDAD) + "',";
//     sql +=      Integer.parseInt(""+tblDat.getValueAt(i, INT_TBL_CODITM)) + ",";
//     sql +=      Integer.parseInt(""+tblDat.getValueAt(i, INT_TBL_CODITM)) + ",'";
//     sql +=      tblDat.getValueAt(i, INT_TBL_ITMALT) + "','"; 
//     sql +=      tblDat.getValueAt(i, INT_TBL_ITMALT2) + "','"; 
//     sql +=      tblDat.getValueAt(i, INT_TBL_DESITM) + "',";
//     sql +=      Integer.parseInt(""+tblDat.getValueAt(i, INT_TBL_CODBOD)) + ",";
//     sql +=      Double.parseDouble(""+tblDat.getValueAt(i, INT_TBL_CANMOV)) + ",";
//     sql +=      objUti.redondeo(Double.parseDouble( (tblDat.getValueAt(i, INT_TBL_PRECOS)==null)?"0":tblDat.getValueAt(i, INT_TBL_PRECOS).toString()),objZafParSis.getDecimalesBaseDatos()) + ",";                                                       
//     sql +=      objUti.redondeo(Double.parseDouble( (tblDat.getValueAt(i, INT_TBL_PRECOS)==null)?"0":tblDat.getValueAt(i, INT_TBL_PRECOS).toString()),objZafParSis.getDecimalesBaseDatos()) + ",";                                                       
//     sql +=      objUti.redondeo(Double.parseDouble( (tblDat.getValueAt(i, INT_TBL_PRECOS)==null)?"0":tblDat.getValueAt(i, INT_TBL_PRECOS).toString()),objZafParSis.getDecimalesBaseDatos()) + ",";                                                       
//     sql +=      objUti.redondeo(Double.parseDouble( (tblDat.getValueAt(i, INT_TBL_PORDES)==null)?"0":tblDat.getValueAt(i, INT_TBL_PORDES).toString()),objZafParSis.getDecimalesMostrar()) + ",'";
//     sql +=   ((tblDat.getValueAt(i, INT_TBL_BLNIVA)==null)?"N":(tblDat.getValueAt(i, INT_TBL_BLNIVA).toString().equals("true"))?"S":"N") + "','S',";
//     sql +=      objUti.redondeo(Double.parseDouble( (tblDat.getValueAt(i, INT_TBL_TOTAL)==null)?"0":tblDat.getValueAt(i, INT_TBL_TOTAL).toString()),objZafParSis.getDecimalesBaseDatos()) + ",";                                                       
//     sql +=      objUti.redondeo(Double.parseDouble( (tblDat.getValueAt(i, INT_TBL_TOTAL)==null)?"0":tblDat.getValueAt(i, INT_TBL_TOTAL).toString()),objZafParSis.getDecimalesBaseDatos()) + ",";                                                       
//     sql +=      objUti.redondeo(Double.parseDouble( (tblDat.getValueAt(i, INT_TBL_TOTAL)==null)?"0":tblDat.getValueAt(i, INT_TBL_TOTAL).toString()),objZafParSis.getDecimalesBaseDatos()) + ",'I', " +
//             " 0, '"+str_MerIEFisBod+"' )"; 
//
//     txtsql.append(sql + "; \n");
//                                                 
...
//        intTipDocFac+","+intCodDocFac+"," +
//        tblDat.getValueAt(i,INT_TBL_NUMREL).toString()+",'A',"+objZafParSis.getCodigoLocal()+","+
//        txtCodTipDoc.getText()+","+intNumDoc+","+(i+1)+","+objZafParSis.getCodigoEmpresa()+")";
//        txtsql.append(sSQL + "; \n");
//
//
//
...
//
//    if( intTipCli==0){
//     //    System.out.println(" EL ITEM ES DE SERVICIO ...... 3  "); //**************
//        if (!objZafInv.movInventario_2_solo_OC_venoc( CONN_GLO, con_remota, objZafParSis.getCodigoEmpresa(),objZafParSis.getCodigoLocal(),objTipDoc.getco_tipdoc(),intNumDoc, Integer.parseInt(""+tblDat.getValueAt(i, INT_TBL_CODITM)), Integer.parseInt(""+tblDat.getValueAt(i, INT_TBL_CODBOD)), dblDif ,Double.parseDouble(tblDat.getValueAt(i,INT_TBL_CANMOV).toString()),Double.parseDouble(tblDat.getValueAt(i,INT_TBL_PRECOS).toString()),Double.parseDouble(tblDat.getValueAt(i, INT_TBL_PORDES).toString()),getAccionDoc(),(objTipDoc.getst_calculacosto().equals("S")?true:false), strEstFisBod,  strMerIngEgr,  strTipIngEgr ) ){
...
//  }  
//    if( intTipCli==1){ 
//      //   System.out.println(" EL ITEM ES DE SERVICIO ...... 4  "); //**************
//       if (!objZafInv.movInventario_solo_OC_venoc( CONN_GLO, con_remota, objZafParSis.getCodigoEmpresa(),objZafParSis.getCodigoLocal(),objTipDoc.getco_tipdoc(),intNumDoc, Integer.parseInt(""+tblDat.getValueAt(i, INT_TBL_CODITM)), Integer.parseInt(""+tblDat.getValueAt(i, INT_TBL_CODBOD)), dblDif ,Double.parseDouble(tblDat.getValueAt(i,INT_TBL_CANMOV).toString()),Double.parseDouble(tblDat.getValueAt(i,INT_TBL_PRECOS).toString()),Double.parseDouble(tblDat.getValueAt(i, INT_TBL_PORDES).toString()),getAccionDoc(),(objTipDoc.getst_calculacosto().equals("S")?true:false), strEstFisBod,  strMerIngEgr,  strTipIngEgr ) ){
//                double dblFalta = objSisCon.getStockConsolidado()-java.lang.Math.abs(dblDif);
//                if (strMsgError.equals("")){                                                            
//                    if (mensaje("**Ocurrio un problema en el inventario \n ¿Desea seguir verificando?")!=javax.swing.JOptionPane.YES_OPTION){
//                        MensajeInf("<html> "+ (i+1)+".-" + tblDat.getValueAt(i, INT_TBL_ITMALT).toString() +"  falta <FONT COLOR=\"blue\">" + dblFalta + "</font> para completar el pedido.</html>"); 
...
//     for(int i=0; i<tblDat.getRowCount(); i++){  //**************
//        if(tblDat.getValueAt(i,INT_TBL_CODITM) != null ){  //********* 
//            if(Double.parseDouble(tblDat.getValueAt(i,INT_TBL_CANORI)==null?"0":tblDat.getValueAt(i,INT_TBL_CANORI).toString()) - Double.parseDouble(tblDat.getValueAt(i,INT_TBL_CANMOV)==null?"0":tblDat.getValueAt(i,INT_TBL_CANMOV).toString())!=0){
...
//
//    if (arlActInv.size()>0){
//        if (!objZafInv.UpdateInventarioMaestro_solo_OC(CONN_GLO, con_remota, objZafParSis.getCodigoEmpresa(), arlActInv)){
//            CONN_GLO.rollback();
//             if(INTCODREGCEN!=0) con_remota.rollback();
//            return false;                                                
//        }
...
//
//    /*********************************************\    
//     * Armando  el query para la actualizacion del*   
//     * campo ne_ultDoc de la tabla tbm_cabTipDoc *
//    \*********************************************/
//
//
//
//    /*    
//     *  Insertando el detalle de pago.
//     */
//    insertDetPag(intNumDoc, "I");
//
//    /*********************************************\    
//     * Armando el Insert para los datos          *   
//     * del asiento de diario                     *
//    \*********************************************/
//
//
//      pstInsOrdCom = CONN_GLO.prepareStatement( txtsql.getText() );
//      pstInsOrdCom.executeUpdate();
//      txtsql.setText("");
//
//
//                                        
//                                      if (!objDiario.insertarDiario_OC(CONN_GLO,objZafParSis.getCodigoEmpresa(),
//                                      objZafParSis.getCodigoLocal(), 
//                                      objTipDoc.getco_tipdoc(), txtDoc.getText(), txtOrdCom.getText(), objUti.parseDate(txtFecDoc.getText(),"dd/MM/yyyy")) )                                     
//                                        {
//                                            CONN_GLO.rollback();
//                                             if(INTCODREGCEN!=0) con_remota.rollback();
//                                            return false;
