 ...
 * @author  Eddye Lino 
 */
public class ZafCom19_TRA extends javax.swing.JDialog 
{
    ...
    final int INT_REL_LOCDOC=21;                       
    final int INT_REL_TIPDOC=22;                   
    final int INT_REL_CODDOC=23;                   
    final int INT_REL_CODREG=24;                          
    ...
    private Connection con, conCab, CONN_GLO=null, CONN_REMOTO=null;
    ...
    Librerias.ZafUtil.UltDocPrint objUltDocPrint;  // Para trabajar con la informacion de tipo de documento 
     ...
     int intCodDocFac=0;
     int intCodLocGrp=0;
     StringBuffer stbItm=new StringBuffer();
     private String Str_RegSel[];
     int intTipMovTrasn=0;
     String STRCODEMPTRANS_GLO="", STRCODLOCTRANS_GLO="", STRCODTIPTRANS_GLO="", STRCODDOCTRANS_GLO=""; 
     int  intTipDocConf_glo=0;
     
       
    /** Crea una nueva instancia de la clase ZafCon06. */    
public ZafCom19_TRA(java.awt.Frame parent, boolean modal, Librerias.ZafParSis.ZafParSis obj, int intCodCot2, int intCodMnu,  java.sql.Connection con_remoto ,  java.sql.Connection con_local, int  intTipDoc , int intCodDoc, StringBuffer stb, int intTipTrasn  ,int intTipDocConf  ) {
 super(parent, modal);
 try{
   initComponents();  
   //Inicializar objetos.
    objParSis=(ZafParSis)obj.clone();
    intCodEmpGrp=objParSis.getCodigoEmpresa(); // objParSis.getCodigoEmpresaGrupo();
    intCodLocGrp=objParSis.getCodigoLocal();
          
    intTipDocConf_glo = intTipDocConf;
    intTipMovTrasn=intTipTrasn;
   ...
   jfrThis = this;
    ...
    /** Crea una nueva instancia de la clase ZafCon06. */    
public ZafCom19_TRA(java.awt.Frame parent, boolean modal, Librerias.ZafParSis.ZafParSis obj, int intCodCot2, int intCodMnu,  java.sql.Connection con_remoto ,  java.sql.Connection con_local, int  intTipDoc , int intCodDoc, StringBuffer stb,int intTipTrasn, String STRCODEMPTRANS, String STRCODLOCTRANS, String STRCODTIPTRANS, String STRCODDOCTRANS  ,int intTipDocConf   ) {
 super(parent, modal);
 try{
   initComponents();  
   //Inicializar objetos.
    ...
    intTipMovTrasn=intTipTrasn;
    STRCODEMPTRANS_GLO=STRCODEMPTRANS;
    STRCODLOCTRANS_GLO=STRCODLOCTRANS;
    STRCODTIPTRANS_GLO=STRCODTIPTRANS;
    STRCODDOCTRANS_GLO=STRCODDOCTRANS;
  ...
  if(intTipMovTrasn==2){ 
          if(isCamVal()){
           if(modificarReg()){
             ...
             String strTit, strMsg;
             strTit="Mensaje del sistema Zafiro";
             strMsg="La operación MODIFICAR se realizó con éxito.";
             //oppMsg.showMessageDialog(jfrThis,strMsg,strTit,javax.swing.JOptionPane.INFORMATION_MESSAGE);
                blnEst = true;
             cerrarObj();
             System.gc(); 
...
public void  refrescaDatos(){
  ...
  int intCodBod, intCodBod2;
  int intVal=0;
  String strSql="", strSqlAux="", strSqlAux2="";
  double dblCan=0;
  double dblCosUni=0;
  
  int intCodBodIngEgr=0;
  
  
  if(intTipDocConf_glo==148){
    if(objParSis.getCodigoEmpresa()==1)   intCodBodIngEgr=5;
    if(objParSis.getCodigoEmpresa()==2)   intCodBodIngEgr=7;
    if(objParSis.getCodigoEmpresa()==4)   intCodBodIngEgr=5;
  }
   
  if(intTipDocConf_glo==149){
    if(objParSis.getCodigoEmpresa()==1)   intCodBodIngEgr=6;
    if(objParSis.getCodigoEmpresa()==2)   intCodBodIngEgr=8;
    if(objParSis.getCodigoEmpresa()==4)   intCodBodIngEgr=6;
  }
 ...
 try{
    if(CONN_GLO!=null){  
     ...
     if(intTipMovTrasn==2){
       ...
       strSql="SELECT  a1.co_tipdoc, a1.tx_descor, a1.tx_deslar, a.co_doc, a.ne_numdoc, a.fe_Doc " +
       ...
       " where a.co_emp="+STRCODEMPTRANS_GLO+" and a.co_loc="+STRCODLOCTRANS_GLO+" and a.co_tipdoc="+STRCODTIPTRANS_GLO+"" +
       " and a.co_doc="+STRCODDOCTRANS_GLO+"   ";
       ...
       while(rstLoc.next()){ 
            ...
            this.txtCodDoc.setText( rstLoc.getString("co_doc") );
            this.txtNumDoc.setText( rstLoc.getString("ne_numdoc") );
            ...
            if(dateObj==null){
              dtpFecDoc.setText("");  
                ...
                dtpFecDoc.setText(calObj.get(java.util.Calendar.DAY_OF_MONTH),
    ...
    if(objParSis.getCodigoMenu()==2063){   
       strSql="SELECT * FROM (  SELECT x.coloc, x.cotipdoc, x.codoc, x.coreg, x.coitm as co_itm, inv.tx_codAlt, inv.tx_nomitm, var.tx_descor, x.ndcan as nd_can, inv.nd_cosuni, a.co_bod, a.tx_nom, a1.co_bod as cobod2," +
       " a1.tx_nom as nombod2 ,(abs(ndcan)*inv.nd_cosuni) as total " +
       " "+strSqlAux2+" FROM ( " +
       stbItm+" "+ 
     " ) as x" +
               ...
               " INNER JOIN tbm_bod AS a ON (a.co_emp=x.coemp AND a.co_bod=x.cobod)  " +
               " INNER JOIN tbm_bod AS a1 ON (a1.co_emp=x.coemp AND a1.co_bod="+intCodBodIngEgr+")  ) AS x  ORDER BY  coreg    ";  
    }  
    if(objParSis.getCodigoMenu()==2073){     
       strSql="SELECT * FROM ( SELECT x.coloc, x.cotipdoc, x.codoc, x.coreg, x.coitm as co_itm, inv.tx_codAlt, inv.tx_nomitm, var.tx_descor, " +
       " x.ndcan as nd_can, inv.nd_cosuni, a.co_bod as cobod2 , a.tx_nom as nombod2, a1.co_bod," +
       " a1.tx_nom ,(abs(ndcan)*inv.nd_cosuni) as total " +
       " "+strSqlAux2+" FROM ( " +
          //     " select 1 as coemp,4 as coloc,148 as cotipdoc, 1 as codoc, 12474 as coitm, 1 as cobod , 45 as ndcan " +
          //     " union all " +
          //     " select 1 as coemp,4 as coloc,148 as cotipdoc, 1 as codoc, 13428 as coitm, 1 as cobod , 5 as ndcan  " +
          //     " union all " +
          //     " select 1 as coemp,4 as coloc,148 as cotipdoc, 1 as codoc, 14135 as coitm, 1 as cobod , 14 as ndcan  " +
       stbItm+" "+ 
     " ) as x" +
               " inner join tbm_inv as inv on (inv.co_emp=x.coemp and inv.co_itm=x.coitm) " +
               " left outer join tbm_var as var on (inv.co_uni = var.co_reg ) " +
        ...
        System.out.println(""+ stbItm.toString() );
       ...
       while(rstLoc.next()){ 
         ...
         intCodBod2=rstLoc.getInt("cobod2");
         
         vecReg.add(INT_TBL_DAT_COD_BOD_ORG, ""+ intCodBod );
         vecReg.add(INT_TBL_DAT_BUT_BOD_ORG,null);
         vecReg.add(INT_TBL_DAT_DEL_BOD_ORG, rstLoc.getString("tx_nom") );
         vecReg.add(INT_TBL_DAT_COD_BOD_DES, ""+ intCodBod2 );
         vecReg.add(INT_TBL_DAT_BUT_BOD_DES,null);
         vecReg.add(INT_TBL_DAT_DEL_BOD_DES, rstLoc.getString("nombod2") );
         ...
         vecReg.add(INT_REL_LOCDOC,  rstLoc.getString("coloc"));
         vecReg.add(INT_REL_TIPDOC,  rstLoc.getString("cotipdoc"));
         vecReg.add(INT_REL_CODDOC,  rstLoc.getString("codoc"));
         vecReg.add(INT_REL_CODREG,  rstLoc.getString("coreg"));
         ...
         public void cargarTipoDoc(){
             ...
             int intCodTipDoc=0;
              try{
                    ...
                    if(objParSis.getCodigoMenu()==2063) intCodTipDoc=150;
                    if(objParSis.getCodigoMenu()==2073) intCodTipDoc=151;
                    ...
                    if(objParSis.getCodigoUsuario()==1){
                              sqlPrede = "Select distinct doc.co_tipdoc,doc.tx_deslar,doc.tx_descor from tbm_cabtipdoc as doc, tbr_tipdocprg as menu " +
                                        " where   menu.co_emp = " +intCodEmpGrp+ " and " +
                                                ...
                                                " menu.co_tipdoc = doc.co_tipdoc and  menu.co_tipdoc="+intCodTipDoc+" ";
                   }else {     
                        sqlPrede  ="SELECT a.co_tipdoc, a.tx_desCor, a.tx_desLar "+
                                    " FROM tbr_tipDocUsr AS a1 inner join  tbm_cabTipDoc AS a ON (a.co_emp=a1.co_Emp and a.co_loc=a1.co_loc and a.co_tipdoc=a1.co_tipdoc)"+
                                    " WHERE "+
                                    "  a1.co_emp=" +intCodEmpGrp+""+
                                    " AND a1.co_mnu=" + intCodMnuPrg +"  and a1.co_tipdoc="+intCodTipDoc+""+
                                    " AND a1.co_usr=" +objParSis.getCodigoUsuario();
             }
                    
                    rstPrede=stmPrede.executeQuery(sqlPrede);
                    
                    System.out.println("sqlPrede: "+sqlPrede);
...
//    }
    ...
    private void initComponents() {
        ...
        lblNumDoc.setBounds(416, 24, 100, 20);
        ...
        panTooBar.setMinimumSize(new java.awt.Dimension(40, 10));
        panTooBar.setPreferredSize(new java.awt.Dimension(60, 45));
        panTooBar.setLayout(new java.awt.BorderLayout());

        butGua.setText("Guardar");
        butGua.setPreferredSize(new java.awt.Dimension(90, 25));
              ...
              if(actualizarTbmInvBod(0)){
                if(VerificaStock()){    
                  if(objAsiDia.insertarDiario_OC( CONN_GLO, intCodEmpGrp, intCodLocGrp, Integer.parseInt(txtCodTipDoc.getText()), txtCodDoc.getText(), txtCodDoc.getText(),  objUti.parseDate(dtpFecDoc.getText(),"dd/MM/yyyy"), 779 )){
                       //recostearItm();
                       blnRes=true;
                   }else{ CONN_GLO.rollback();  if(CONN_REMOTO!=null) CONN_REMOTO.rollback(); } 
                 }else {  CONN_GLO.rollback();  if(CONN_REMOTO!=null) CONN_REMOTO.rollback(); } 
                }else {  CONN_GLO.rollback();  if(CONN_REMOTO!=null) CONN_REMOTO.rollback(); } 
               }else {  CONN_GLO.rollback();  if(CONN_REMOTO!=null) CONN_REMOTO.rollback(); } 
             }else {  CONN_GLO.rollback();  if(CONN_REMOTO!=null) CONN_REMOTO.rollback(); }
...
private boolean invertirTrans(){
  boolean blnRes=false;   
  
 if(objParSis.getCodigoMenu()==2063){
  blnRes=invertirTrans_1();
 }
 if(objParSis.getCodigoMenu()==2073){
  blnRes=invertirTrans_2();
 }
 ...
 private boolean invertirTrans_1(){
  boolean blnRes=false;
  java.sql.Statement stmLoc;
  java.sql.ResultSet rstLoc;
  int intCodBod=0;
  ...
  if(intTipDocConf_glo==148){
    if(objParSis.getCodigoEmpresa()==1)   intCodBod=5;
    if(objParSis.getCodigoEmpresa()==2)   intCodBod=7;
    if(objParSis.getCodigoEmpresa()==4)   intCodBod=5;
  }
   
  if(intTipDocConf_glo==149){
    if(objParSis.getCodigoEmpresa()==1)   intCodBod=6;
    if(objParSis.getCodigoEmpresa()==2)   intCodBod=8;
    if(objParSis.getCodigoEmpresa()==4)   intCodBod=6;
  }
  ...
  try{
     if(CONN_GLO!=null) {
         ...
         " where co_emp="+STRCODEMPTRANS_GLO+" and co_loc="+STRCODLOCTRANS_GLO+" and co_tipdoc="+STRCODTIPTRANS_GLO+" and co_doc="+STRCODDOCTRANS_GLO+" and nd_can < 0";
         rstLoc=stmLoc.executeQuery(strSQL);
         strSQL="";
         while(rstLoc.next()){
          ...
          if(CONN_REMOTO!=null){
                java.sql.Statement stmloc=CONN_REMOTO.createStatement();
          ...
          " WHERE co_emp="+STRCODEMPTRANS_GLO+" and co_loc="+STRCODLOCTRANS_GLO+" and co_tipdoc="+STRCODTIPTRANS_GLO+" and co_doc="+STRCODDOCTRANS_GLO+" ; ";
          strSQL+=" DELETE FROM tbm_detdia " +
          " WHERE co_emp="+STRCODEMPTRANS_GLO+" and co_loc="+STRCODLOCTRANS_GLO+" and co_tipdoc="+STRCODTIPTRANS_GLO+" and co_dia="+STRCODDOCTRANS_GLO+" ; ";
          strSQL+=" DELETE FROM tbm_cabdia " +
          " WHERE co_emp="+STRCODEMPTRANS_GLO+" and co_loc="+STRCODLOCTRANS_GLO+" and co_tipdoc="+STRCODTIPTRANS_GLO+" and co_dia="+STRCODDOCTRANS_GLO+" ; ";
         
          strSQL+="UPDATE tbm_cabmovinv SET nd_sub="+objUti.codificar((objUti.isNumero(txtMonDoc.getText())?"" + (intSig*Double.parseDouble(txtMonDoc.getText())):"0"),3)+"" +
          ", nd_tot="+objUti.codificar((objUti.isNumero(txtMonDoc.getText())?"" + (intSig*Double.parseDouble(txtMonDoc.getText())):"0"),3)+", " +
 ...
 private boolean invertirTrans_2(){
  boolean blnRes=false;
  java.sql.Statement stmLoc;
  java.sql.ResultSet rstLoc;
  int intCodBod=0;
  ...
  return blnRes;
    ...
    private boolean configurarFrm()
    {
        ...
        try
        {
            ...
            vecCab.add(INT_REL_LOCDOC, "Loc.Rel"); 
            vecCab.add(INT_REL_TIPDOC, "TipDoc.Rel"); 
            vecCab.add(INT_REL_CODDOC, "CodDoc.Rel"); 
            vecCab.add(INT_REL_CODREG, "CodReg.Rel"); 
...
//                           intUltSecGrp = objUltDocPrint.getNumeroOrdenGrupo(CONN_GLO);
                ...
                txtNumDoc.setText("" + intNumDoc_Tra);
               ...
               Str_RegSel[0] = ""+intCodEmpGrp;
               Str_RegSel[1] = ""+intCodLocGrp;
               Str_RegSel[2] = txtCodTipDoc.getText();
               Str_RegSel[3] = ""+intUltReg;
...
//                }
            ...
            if (CONN_GLO!=null)
            {
                ...
                for (i=0;i<objTblMod.getRowCountTrue();i++)
                {
                   ...
                   //***********   VERFIFICA SI EL ITEM IE MER FIS BOD  ************************/  
                    ...
                    " ";
                   if(objParSis.getCodigoMenu()==2063){  
                         strSQL+=", co_locrelsolsaltemmer, co_tipdocrelsolsaltemmer, co_docrelsolsaltemmer, co_regrelsolsaltemmer  )";     
                   }else strSQL+=" ) ";
                    ...
                    if(objParSis.getCodigoMenu()==2063){  
                      strSQL+=", "+objTblMod.getValueAt(i,INT_REL_LOCDOC);
                      strSQL+=", "+objTblMod.getValueAt(i,INT_REL_TIPDOC);
                      strSQL+=", "+objTblMod.getValueAt(i,INT_REL_CODDOC);
                      strSQL+=", "+objTblMod.getValueAt(i,INT_REL_CODREG);
                    }
                    strSQL+=" )";
                    ...
                    stm.executeUpdate(strSQL);
                     ...
                     if(!(strSQLFisBod.equals(""))) stm.executeUpdate(strSQLFisBod);
                      ...
                      if(CONN_REMOTO!=null) {  
                        //System.out.println("Not Null.. ");    
                         java.sql.Statement stm2=CONN_REMOTO.createStatement();
    ...
    private boolean VerificaStock(){
        ...
        try{  
               ...
               //**********************  STOCK DEL ITEM **************************//
                     if (i>0)
                        ...
                        stb.append("SELECT a2.nd_stkAct , a2.co_itm "+
                                   ...
                                   " WHERE a2.co_emp="+objParSis.getCodigoEmpresa()+" AND a2.co_bod="+objTblMod.getValueAt(i,INT_TBL_DAT_COD_BOD_ORG)+" " +
         ...
         int Estado=0;
         if(CONN_REMOTO!=null){
            stm=CONN_REMOTO.createStatement();
