...
public class UltDocPrint {
        ...
        public int getUltDoc(int intCodTipDoc){
         ...
         public int getUltDoc(int intCodTipDoc, java.sql.Connection conDoc, int intCodLoc){
            int intUltDoc = -1;
               try{
                    String strSql = "SELECT ne_ultDoc as ultdoc FROM tbm_cabTipDoc where " +
                                  " co_emp = " + objZafParSis.getCodigoEmpresa() + 
                    ...
                    if(rstDoc.next()){
                        ...
                        strSql ="UPDATE tbm_cabTipDoc SET  ne_ultDoc = "+(intUltDoc+1)+
                                "  WHERE co_emp = "+objZafParSis.getCodigoEmpresa()+" and "+  
                                " co_loc = " +intCodLoc+ " and co_tipDoc = " +intCodTipDoc;      
                        stmDoc.executeUpdate(strSql);   
