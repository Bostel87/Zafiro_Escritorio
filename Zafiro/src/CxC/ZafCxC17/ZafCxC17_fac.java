 ...
 * @author  jayapata
 */
public class ZafCxC17_fac extends javax.swing.JDialog {
    ...
    public ZafCxC17_fac(java.awt.Frame parent, boolean modal, Librerias.ZafParSis.ZafParSis obj, int intCod_Emp, String strCocLoc, String strDocTipDoc, String strCocDoc ) {
         super(parent, modal);
        try{ /**/
...
private class ButBod extends Librerias.ZafTableColBut.ZafTableColBut_uni{
    ...
    public void butCLick() {
       ...
       if (intCodTipDoc==1)
       { 
         System.out.println("strDocTipDoc  fac=  "+strDocTipDoc);
         llamarVentanaFac(intCol);
       }
       else
       {
         System.out.println("intCol=  "+intCol); 
         MensajeInf("No hay factura...");   
       }
    }
}
...
private boolean cargarFac(){
 ...
 try{
   ...
   if(conn!=null){
      ...
      System.out.println("cargarFac()= " + strSql );
...
private boolean cargarFacDet(java.sql.Connection conn, java.sql.ResultSet rstDat){
 ...
 try{
   if(conn!=null){
       ...
       System.out.println("cargarFacDet()= " + strSql );
       rstLoc=stmLoc.executeQuery(strSql);
       while(rstLoc.next()){
...
private boolean cargarFacPag(java.sql.Connection conn, java.sql.ResultSet rstDat){
 ...
 try{
   if(conn!=null){
       ...
       System.out.println("cargarFacPag()= " + strSql );
       rstLoc=stmLoc.executeQuery(strSql);
       while(rstLoc.next()){
          java.util.Vector vecReg = new java.util.Vector();
